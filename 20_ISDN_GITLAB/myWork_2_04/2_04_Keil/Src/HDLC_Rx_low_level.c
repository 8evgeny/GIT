...
char telegram_string[TELEGRAM_LENGTH];
uint8_t telegram_len = 0;
uint8_t flag_telegram_received = 0;
#ifdef DEBUG_INFO
static char str[150];
#endif
...
void processHdlcFrame(void) {
	...
	if (flag_hdlc_frame_received) {
		...
		for(i = 0; i < n_bits; i++) { // удаляем бит-стаффинг (лишние нули после 5 единиц)
			if (ones_count >= 5) {
				...
				if (hdlc_frame_bit_sequence[i] == 0) { // если после 5-ти единиц идет 0, удаляем его
					...
					puts("\r\r> 5 ones contiguous!\r"); // по идее, такие ситуации фильтруются ещё в BitRingProcessBit()
		...
		if ((n_bits % 8) != 0) { // содержимое кадра HDLC должно быть кратно 8 битам
          ...
          telegram_len = tel_len;
					puts("->HDLC Rx: ");
					dbgUartPutsArray(telegram_string, tel_len);
					puts("\r");
					flag_telegram_received = 1;
          CLR_WDT();
				} else {
					puts("Telegram is too long!\r");
				}
				#ifdef DEBUG_INFO
				puts("   ");
				puts("CRC OK\r");
