...
bool heapTest()
{
  ...
  timer.start ();
  heap::amazonHeap &test_heap = heap::amazonHeap::instance ();
  time_elapsed = timer.get_elapsed_ns ();
  size_t heap_size = test_heap.xPortGetFreeHeapSize ();
  std::cout << "heap init@ " << time_elapsed << " nanosecs\n";
  std::cout << "heap:\nSize: " << heap_size << "\nMinimum ever free heap size: "
      << test_heap.xPortGetMinimumEverFreeHeapSize () << "\n";

  for(int i = 1;i < heap_size/2;i++)
    {
      testAmazonHeapPice (test_heap, i);
    }
  //testAmazonHeapPice (test_heap, heap_size + 1000);
  return true;
}
