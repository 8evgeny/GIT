...
// shortcut 4, compute MD5 for string and finalize it
...
// shortcut 4, compute MD5 for CID# and finalize it
MD5::MD5(uint16_t in) {
  init();
  update(in);
  finalize();
...
// for convenience provide a verson with signed char, and single 8-,16-,32-bit
// values
void MD5::update(const char input[], size_type length) {
  update((const unsigned char *)input, length);
}

void MD5::update(uint8_t in) { update((const unsigned char *)&in, 1); }

void MD5::update(uint16_t in) { update((const unsigned char *)&in, 2); }

void MD5::update(uint32_t in) { update((const unsigned char *)&in, 4); }
...
uint8_t *MD5::getMD5sum() {
  finalize();
  return digest;
}

void MD5::getMD5sum(uint8_t *out) {
  finalize();
  std::memcpy(out, digest, MD5_SIZE);
}
