/**
  ...
  * @file    g711.c
  * @author  dmib@@ETS
  * @brief   This file contains G.711 codec implementation
  ******************************************************************************
  */
...
/**
  * @brief  G711 i16 to u-Law table
  */
const unsigned char g711_l2u[4096] = {
...
/**
  * @brief  G711 i16 to A-Law table
  */
const unsigned char g711_l2A[2048] = {
...
/**
  * @brief  G711 u-Law to i16 table
  */
const short g711_u2l[256] = {
...
/**
  * @brief  G711 A-Law to i16 table
  */
const short g711_A2l[256] = {
...
void g711_pcm2alaw(short *src,unsigned char *dst,unsigned int nsmpl)
{
  ...
  while (nsmpl--) {
	tmp=(*src++);
	mask = (tmp < 0) ? 0x7f : 0xff;
	if (tmp < 0) tmp = -tmp;
	tmp >>= 4;
	(*dst++)= g711_l2A[tmp] & mask;
  }
}
...
void g711_pcm2ulaw(short *src,unsigned char *dst,unsigned int nsmpl)
{
  ...
  while (nsmpl--) {
	...
	mask = (tmp < 0) ? 0x7f : 0xff;
	if (tmp < 0) tmp = -tmp;
	if (tmp < 4) (*dst++)= 0xff & mask;
	else {
	 tmp -= 4;
	 tmp >>= 3;
	 (*dst++)= g711_l2u[tmp] & mask;
	}
  }
}
