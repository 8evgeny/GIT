/**
  ...
  * @file    AES_codec.c
  * @author  dmib@@ETS
  * @brief   This file contains AES coder and decoder
  ******************************************************************************
  */
...
/**
  * @brief  AES cyclic left shift macros
  */
#define rol(x,n) (((x)<<n) | ((x)>>(32-(n))))

/**
  * @brief  AES standart precalculated sbox table
  */
...
/**
  * @brief  AES standart precalculated inverse sbox table
  */
...
/**
  * @brief  AES Precalculated table (encryption) MixColumns
...
/**
  * @brief  AES Precalculated table (decryption) InvMixColumns
...
/**
  * @brief  AES Round constant word array
  * 
  */
...
void AES_key_init(AES_State *st, uint8_t *key)
{
  ...
  st->Nk = 4;
  st->Nr = 10; 
#elif (AES_TYPE == AES_192 )
  st->Nk = 6;
  st->Nr = 12; 
#elif (AES_TYPE == AES_256 )
  st->Nk = 8;
  st->Nr = 14; 
#else
#error "AES_TYPE not defined"
#endif
...
/**
  * @brief  AES block encrypting
  * @param  [in] st - AES state structure pointer
  * @param  [inout] block - array of bytes for encrypting
  * @note   st must be already initialized
  * @note   block must be AES_BLOCK_SIZE size
  * @retval None
  */  
void AES_Code_Block(uint8_t *block, AES_State *st)
{
...
/**
  * @brief  AES block decrypting
  * @param  [in] st - AES state structure pointer
  * @param  [inout] block - array of bytes for decrypting
  * @note   st must be already initialized
  * @note   block must be AES_BLOCK_SIZE size
  * @retval None
  */  
void AES_Decode_Block(uint8_t *block, AES_State *st)
{
  uint32_t s0,s1,s2,s3;
  uint32_t t0,t1,t2,t3,tmp;
  uint8_t i, nr;
...
void AES_code_data(AES_State *st, uint8_t *data_in, uint8_t *data_out, uint32_t len)
{
 ...
 while (len >= AES_BLOCK_SIZE) {
	 memcpy(block,data_in,AES_BLOCK_SIZE);
	 AES_Code_Block(block,st);
	 memcpy(data_out,block,AES_BLOCK_SIZE);
	 
	 data_in += AES_BLOCK_SIZE;
	 data_out += AES_BLOCK_SIZE;	 
	 len -= AES_BLOCK_SIZE;
 }
}
...
/**
  * @brief  Global static AES state structure
  */
AES_State aes_codec;

/**
  * @brief  Get static AES state structure
  * @retval pointer to global static AES state structure
  */  
