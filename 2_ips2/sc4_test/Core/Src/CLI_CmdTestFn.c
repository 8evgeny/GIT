...
void fnTestEEPROM(int numParam, char** params)
{
	...
	if (numParam>0) {
		if (!strcmp(params[0],"FORMAT")) {
			...
			res = eeprom_write_bytes(0, (uint8_t *)eeprom_hdr, sizeof(SystemBlockHdr));
			if (res == HAL_OK) {
			 res = eeprom_waitwrite_polling(0,&delay);
			 if (res == HAL_OK) CLI_print("EEPROM formatted, write delay %i\r\n",delay);
			}
			if (res != HAL_OK) {
...
void test_print_TestStartInfo()
{
	...
	CLI_print("MAX address :%.2X-%.2X-%.2X-%.2X-%.2X-%.2X\r\n",
