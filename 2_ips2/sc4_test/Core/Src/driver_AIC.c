...
#define EC_MIC_ENABLE_ENERGY_LEVEL     1000000
// number of SAI_DMA_BUFFER buffers to fade
#define EC_FADE_CNT	12
...
void sound_dampen(int16_t *sndbuf)
{
	uint16_t i;
	for (i=0;i<SAI_DMA_BUFFER_SIZE;i++)
	 sndbuf[i] = sndbuf[i] >> EC_FADE_PARAM;
}
#endif  // AIC_ECHO_CANCEL_ENABLE
...
void aic_task()
{
	...
	if (readytxbuf!=0xFF)
	{
		...
		if (sai_loop_on) {
			...
			if (ec_GR_disable) {
			    if (eng_MIC > EC_MIC_ENABLE_ENERGY_LEVEL) {
				  ec_MIC_disable = 0;
				} else {
				  if ((!ec_MIC_disable) && (!ec_MIC_time)) ec_MIC_time = EC_FADE_CNT;
				}
				if (ec_MIC_time) {
					ec_MIC_time--;
				    if (!ec_MIC_time) ec_MIC_disable = 1;
				}
			}

			if (eng_GR > EC_GR_ENABLE_ENERGY_LEVEL) {
			    if (ec_GR_disable&(!ec_GR_time)) {
			    	ec_GR_time = (EC_FADE_CNT>>2);
			    	ec_MIC_disable = 1;
			    }
			} else {
			    if ((!ec_GR_disable)&(!ec_GR_time)) {
			    	ec_GR_time = EC_FADE_CNT;
			    }
			}

			if (ec_GR_time) {
			    ec_GR_time--;
			    if (!ec_GR_time) ec_GR_disable ^= 1;
			}
