...
{
    char *md5str = (char *) std::malloc(sizeof(char) * (len * 2 + 1));
...
# define SET(n) \
	...
	(u32plus)ptr[(n) * 4] | \
	((u32plus)ptr[(n) * 4 + 1] << 8) | \
	((u32plus)ptr[(n) * 4 + 2] << 16) | \
	((u32plus)ptr[(n) * 4 + 3] << 24))
# define GET(n) \
	(ctx->block[(n)])
#endif
...
const void *MD5::body(void *ctxBuf, const void *data, size_t size)
{
	...
	u32plus a, b, c, d;
	u32plus saved_a, saved_b, saved_c, saved_d;
...
void MD5::Update(void *ctxBuf, const void *data, size_t size)
{
    CTX *ctx = (CTX *)ctxBuf;
	u32plus saved_lo;
	u32plus used, free;
	...
	if (used) {
		...
		std::memcpy(&ctx->buffer[used], data, free);
...
void MD5::Final(unsigned char *result, void *ctxBuf)
{
    CTX *ctx = (CTX *)ctxBuf;
	u32plus used, free;
	...
	if (free < 8) {
		std::memset(&ctx->buffer[used], 0, free);
		body(ctx, ctx->buffer, 64);
		used = 0;
		free = 64;
	}

	std::memset(&ctx->buffer[used], 0, free - 8);
...
unsigned char *MD5::make_hash1(char *arg)
{
	return make_hash2(arg, std::strlen(arg));
}

unsigned char *MD5::make_hash2(const void *data, size_t size)
{
	CTX context;
	unsigned char *hash = (unsigned char *) std::malloc(16);
	Init(&context);
