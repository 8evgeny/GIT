...
/** @addtogroup ADC_DMA_ping_pong ADC_DMA_ping_pong
  * @{
  */
...
void DMA_IRQHandler(void)
{
    ...
    {
        if((MDR_DMA->CHNL_PRI_ALT_SET & (1<<DMA_Channel_ADC)) == (0<<DMA_Channel_ADC))     // The main structure is now in progress
        {
            DMA_CtrlDataInit(&DMA_AltCtrlStr_ADC, &DMA_ControlTable[DMA_Channel_ADC + 32]);
        }
        else if((MDR_DMA->CHNL_PRI_ALT_SET & (1<<DMA_Channel_ADC)) == (1<<DMA_Channel_ADC)) // The alternative structure is now in progress
        {
...
/** @} */ /* End of group ADC_DMA_ping_pong */
