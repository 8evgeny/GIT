...
#define DEMO_EMBEDDEDWIZARD_ADDRESS    0x08040000
#define DEMO_STEMWIN_ADDRESS           0x080F0000
#define DEMO_TGFX_ADDRESS              0x08170000  

#define DEMO_HOME_SIGNATURE            0x5AA55BBB
#define DEMO_EMBEDDEDWIZARD_SIGNATURE  0x5AA55CCC
#define DEMO_TGFX_SIGNATURE            0x5AA55AAA
#define DEMO_STEMWIN_SIGNATURE         0x5AA55DDD
/* USER CODE END PD */
...
static __IO uint32_t DemoSignature = 0;
/* USER CODE END PV */
...
static void ReadBackupRegisters( void )
{
  ...
  thirdPartyAdress  = RTC->BKP30R;
  DemoSignature     = RTC->BKP31R;
...
static void Set_DemoSignature( void )
{
  ...
  RTC->BKP31R = DEMO_HOME_SIGNATURE;

  /* Disable RTC back-up registers access */
  HAL_PWR_DisableBkUpAccess();
}

static void Set_DemoAddress(uint32_t location)
{
  /* Enable RTC back-up registers access */
  HAL_PWR_EnableBkUpAccess();
...
static void Reset_DemoAddress(void)
{
  Set_DemoAddress(0);
}
...
static void GoTo_SubDemo(const uint32_t thirdPartyAdress)
{
...
void GoTo_EmbeddedWizard(void)
{
  GoTo_SubDemo(DEMO_EMBEDDEDWIZARD_ADDRESS);
}

void GoTo_STemWin(void)
{
  GoTo_SubDemo(DEMO_STEMWIN_ADDRESS);
}

void GoTo_TouchGFX(void)
{
  GoTo_SubDemo(DEMO_TGFX_ADDRESS);
}
...
int main(void)
{
  ...
  *((uint32_t*)0x51008108) = 1; /* AXI SRAM issue in cut 1.2 */
  ...
  /* Store Demo Signature */
  Set_DemoSignature();

  if ( thirdPartyAdress != 0 ) // Jump to Embedded wizard
  {
    /* Reset the content of the backup 0 register */
    Reset_DemoAddress();
...
void SystemClock_Config(void)
{
  ...
  */
  ...
  PeriphClkInitStruct.PeriphClockSelection = RCC_PERIPHCLK_LTDC|RCC_PERIPHCLK_QSPI
                              |RCC_PERIPHCLK_FMC;
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOA, LCD_BL_CTRL_Pin|RENDER_TIME_Pin, GPIO_PIN_SET);
