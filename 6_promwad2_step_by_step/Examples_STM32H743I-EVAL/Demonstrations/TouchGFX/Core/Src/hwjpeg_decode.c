...
static osMessageQId DecodeEvent = 0;
...
void HAL_JPEG_MspInit(JPEG_HandleTypeDef *hjpeg)
{
  ...
  hmdmaIn.Init.SourceBurst        = MDMA_SOURCE_BURST_SINGLE;
  hmdmaIn.Init.DestBurst          = MDMA_DEST_BURST_SINGLE;
  ...
  hmdmaOut.Init.SourceBurst     = MDMA_SOURCE_BURST_SINGLE;
  hmdmaOut.Init.DestBurst       = MDMA_DEST_BURST_SINGLE;
  ...
  hmdmaOut.Init.DestBurst = MDMA_DEST_BURST_8BEATS;
...
{
    if(!DecodeEvent) /* decoder not already initialized */
    {
...
int JPEG_Decode_DMA(struct HwJPEG_Context_s *context, uint8_t *buffer,
                    uint32_t length, uint32_t MCUBufferAddress, uint32_t DestAddress, uint32_t DestPitch)
{
        ...
        if (event.status == osEventMessage) {
            if (event.value.v == DECODE_DECODE_COMPLETE_MESSAGE) {
                ...
                DMA2D_YCbCr_To_RGB_2((uint32_t*)MCUBufferAddress,
                                     ...
                                     (uint8_t*)DestAddress,
                                     context->width, context->height, DestPitch);
                return 0;
            } else
