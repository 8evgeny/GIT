...
BumpMapImage::BumpMapImage(BitmapId image_, const unsigned int* bump_map_, const unsigned char* light_source_) :
    ...
    image(image_),
    bump(BITMAP_TOUCHGFX_LIGHT_EFFECT_ID),
    bump_map(bump_map_),
...
{
    currentTime += 2;
    uint16_t tempPositionX = (int)(180.0f * cosf((float)   currentTime / 64)) + 300;
    uint16_t tempPositionY = (int)(160.0f * sinf((float) - currentTime / 45)) + 200;
