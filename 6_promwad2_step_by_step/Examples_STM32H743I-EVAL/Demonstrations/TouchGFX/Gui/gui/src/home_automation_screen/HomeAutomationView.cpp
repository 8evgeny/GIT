...
void HomeAutomationView::setupScreen()
{
    ...
    mainScreenButton[2].setBitmaps(Bitmap(BITMAP_MAIN_BUTTON_STATISTICS_ID), Bitmap(BITMAP_MAIN_BUTTON_STATISTICS_PRESSED_ID));
...
void HomeAutomationView::animateMainMenuButtonToCenter(int index)
{
    for (int i = 0; i < NUMBER_OF_MAIN_SCREEN_BUTTONS; i++)
    {
        if (i == index)
        {
            ...
            mainScreenButton[i].startMoveAnimation(328 - 81 - mainScreenButtonContainer.getX(), mainScreenButton[i].getY(), 12, EasingEquations::cubicEaseInOut);
        }
        else
        {
...
{
    return (mainScreenButton[0].getWidth() + 37) * index;
}
...
void HomeAutomationView::screenClick(ClickEvent::ClickEventType type, int x, int y)
{
    ClickEvent temp_evt(type, x, y);
    View::handleClickEvent(temp_evt);
}

void HomeAutomationView::screenDrag(int16_t x_old, int16_t y_old, int16_t x_new, int16_t y_new)
{
    DragEvent temp_evt(DragEvent::DRAGGED, x_old, y_old, x_new, y_new);
    View::handleDragEvent(temp_evt);
}

void HomeAutomationView::screenCircleUpdate(int16_t degree)
{
    roomControlAdjustable.updateControlWheel(degree);
}
