...
namespace touchgfx
{
    ...
    MJPEGReader::MJPEGReader()
        : movieEndedAction(0), widget(0), frameBuffer(0), temporaryFrameBuffer(0), lineBuffer(0),
          ...
          firstFrameOffset(0), indexOffset(0), playOffset(0)
    {
#ifdef SIMULATOR
        useHWDecoding = false;
#else
...
#ifdef SIMULATOR
#if USE_BPP == 16
        memset(frameBuffer, 0, outputWidth * outputHeight * 2);
#else
    ...
    bool MJPEGReader::showNextFrame(bool show)
    {
        ...
        readData(offset, 8);
        if ((lastError == AVI_NO_ERROR) && (fileBuffer != 0))
        {
            ...
            if (streamNo == STREAM0 && chunkType == TYPEDC)
            {
                ...
                if (show)
                {
                    ...
                    Bitmap::BitmapFormat format = Bitmap::RGB565;
#elif USE_BPP==24
                    Bitmap::BitmapFormat format = Bitmap::RGB888;
#endif
                    if (!decodeFrame(offset, frameBuffer, format))
                    {
    ...
    bool MJPEGReader::decodeThumbNail(Bitmap bmp, uint32_t frameNumber)
    {
        ...
        if ((width != frameWidth) || (height != frameHeight))
        {
            ...
            if (!decodeFrame(offset, temporaryFrameBuffer, Bitmap::RGB888, 0))
            {
                return false;
            }
        ...
        else
        {
            ...
            Bitmap::BitmapFormat format = Bitmap::RGB888;
#endif
            return decodeFrame(offset, buffer, format, 0);
        }

        return true;
    }

    bool MJPEGReader::decodeFrame(uint32_t offset, uint8_t* const buffer, Bitmap::BitmapFormat format, int mode)
    {
#ifndef SIMULATOR
      if (useHWDecoding && mode)
      {
        ...
        if(JPEG_Decode_DMA(&HwJPEG_Context, fileBuffer, dataLength, (uint32_t)temporaryFrameBuffer, (uint32_t)buffer, frameWidth) < 0)
        {
            return false;
        }
      }
...
#endif
      {
        ...
        if (lastError == AVI_NO_ERROR)
        {
            ...
            while (cinfo.output_scanline < frameHeight)
            {
                ...
                //loop row
                if (format == Bitmap::RGB565) {
                    uint16_t* dest = (uint16_t*)lineptr;
                    uint8_t* src = lineBuffer;
                    for (uint32_t counter = 0; counter < frameWidth; counter++) {
                        uint16_t pixel;
                        pixel = (*src++ & 0xf8) << 8;
                        pixel |= (*src++ & 0xfc) << 3;
                        pixel |= (*src++ & 0xf8) >> 3;
