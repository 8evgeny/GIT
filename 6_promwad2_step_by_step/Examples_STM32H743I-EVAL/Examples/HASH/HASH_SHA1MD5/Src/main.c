...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
#define INPUT_TAB_SIZE    ((uint32_t) 261)/* The size of the input data "Input" */
...
static uint8_t MD5Digest[16];
static uint8_t ExpectMD5Digest[16] = {0x67, 0x07, 0x86, 0x2c, 0x7c, 0xd0, 0xb5, 0x22,
                                       0xb2, 0xdd, 0x22, 0xd8, 0x47, 0x7b, 0xe3, 0x18};
static uint8_t SHA1Digest[20];
static uint8_t ExpectSHA1Digest[20] = {0xf8, 0x59, 0xc1, 0x8d, 0xec, 0x94, 0x72, 0x42,
...
int main(void)
{
  ...
  /* Compute SHA1 */
  if (HAL_HASH_SHA1_Start(&HashHandle, (uint8_t *)Input, INPUT_TAB_SIZE, SHA1Digest, 0xFF) != HAL_OK)
  {
    Error_Handler();
  }
  /* Compare computed digest with expected one */
  if (memcmp(SHA1Digest, ExpectSHA1Digest, sizeof(ExpectSHA1Digest) / sizeof(ExpectSHA1Digest[0])) != 0)
  {
    BSP_LED_On(LED3);
  }
  else
  ...
  /* Compute SHA1 */
  if (HAL_HASH_MD5_Start(&HashHandle, (uint8_t *)Input, INPUT_TAB_SIZE, MD5Digest, 0xFF) != HAL_OK)
  {
    Error_Handler();
  }
  /* Compare computed digest with expected one */
  if (memcmp(MD5Digest, ExpectMD5Digest, sizeof(ExpectMD5Digest) / sizeof(ExpectMD5Digest[0])) != 0)
  {
    Error_Handler();
  }
  else
