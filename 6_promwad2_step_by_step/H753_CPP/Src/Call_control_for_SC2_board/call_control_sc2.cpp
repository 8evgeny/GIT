    ...
    for(uint8_t i = 0; i <size; ++i ) {
        ...
        if (incomingCallType == Conference) {
            ...
            if (getKeysRingBuffData(Assign, i) == ownId && linkMode == Duplex) {
                ...
                GPIO::getInstance()->configLed(getKeysRingBuffData(Key, i), true, 250, 250);
...
//            GPIO::getInstance()->configLed(Asterisk, true, 250, 250);
...
//            assignedKey = Asterisk;
    ...
    if (noFoundKeyFlag) {
        ...
        if (incomingCallType != Telephone) {
            ...
            GPIO::getInstance()->configLed(getKeysRingBuffData(Key, numberOfKey), true, 250, 250);
...
//                    if ( *(missedCallIter + j) == getKeysRingBuffData(Key, numberOfKey)) {
...
//        break;
    ...
    if (!noFoundKeyFlag && incomingCallType != Conference && incomingCallType != Telephone) {
...
void Call_Control::fsmRecvControl(JsonDocument &doc)
{
    ...
    case STATE_RECV_UDP:
        if (Json::getInstance()->thisStation.id == distId && linkStatus == 0xFF) {
            ...
            prevOwnId = ownId;
            osMutexWait	( mutexIncomingCall_id, osWaitForever);
            incomingCall = true;
            osMutexRelease(	 mutexIncomingCall_id);
            waitingThread_Init();
    ...
    case STATE_USER_WAITING:
        if (Json::getInstance()->thisStation.id == distId && linkStatus == 0x00 && prevOwnId == ownId) {
            ...
            if (incomingCallType != Telephone) {
                ...
                GPIO::getInstance()->configLed( assignedKey, true, 100, 900);
                ...
                GPIO::getInstance()->configLed( assignedKey, false);
    ...
    case STATE_ANSWER_WAITING:
        if (Json::getInstance()->thisStation.id == ownId && linkStatus == 0xFF) {
            ...
            if (incomingCallType == Telephone)
                ...
                GPIO::getInstance()->configLed(Asterisk, true);
            ...
            GPIO::getInstance()->configLed(pressedKey, true, 900, 100);
                ...
                if (n == distId) {
                    ...
                    for (uint16_t i = 0; i < size; ++i) {
                        if (getKeysRingBuffData(Assign, i) == distId) {
                            GPIO::getInstance()->configLed(getKeysRingBuffData(Key, i), true, 300, 300, 2);
                            break;
                        }
                    }
                    break;
...
void Call_Control::sendJson(JsonDocument &doc, const size_t capacity)
{
    osMutexWait(Call_Control::getInstance()->mutex_outputJson_id, osWaitForever);
...
//        sramReadBuff = new uint32_t[Json::getInstance()->fileSize];
//        SRAM::getInstance()->readData(sramReadBuff, Json::getInstance()->fileSize, (uint32_t *)0x60000000);

//        DynamicJsonDocument sramDoc(Json::getInstance()->modifiedfileSize);  //memory allocating
//        Json::getInstance()->deserialize(sramDoc, (void*)sramReadBuff, sizeof(uint32_t) * Json::getInstance()->fileSize);


//        uint16_t  numStations = sramDoc["groupsAll"][key.assign /*-1*/]["stations"].size(),
//                  j = 0,
//                  size = numStations < 100 ? numStations : 100;

//        doc["ownId"] = Json::getInstance()->thisStation.id;
//        for (uint16_t i = 0; i < size; ++i, ++j) {
//            doc["distId"].add(sramDoc["groupsAll"][key.assign /*-1*/]["stations"][j]["id"]);
//        }
...
void Call_Control::hangUp(HangUpControl control)
{
    if (callType != NoType) {
        ...
        if (callType == Telephone)
            ...
            GPIO::getInstance()->configLed(pressedKey, false);

    } else GPIO::getInstance()->configLed(assignedKey, false);
    ...
    for(uint8_t i = 0; i < size; ++i ) {
        ...
        for(auto &n : dynamicStoreKeysBuff) {
            if (getKeysRingBuffData(Assign, i) == n) {
                ...
                GPIO::getInstance()->configLed(getKeysRingBuffData(Key, i), false);
    ...
    for(uint8_t i = 0; i < sizeOfKeyRingBuff; ++i) {
        ...
        if (getKeysRingBuffData(Key, i) == key && sizeDynamicStore <= 5) {
            ...
            if (confHangUp) {
                ...
                GPIO::getInstance()->configLed(key, false);
                dynamicStoreKeysBuff.erase(dynamicStoreIter + n);
            } else {
                GPIO::getInstance()->configLed(key);
...
void Call_Control::fsmKeysControl(uint8_t key)
{

    switch (callType) {
    case Direct:
        if (pressedKey == key && keyMode == NoFixed)
            pushToTalk();
        else if (pressedKey == key && keyMode == Fixed) {
            hangUp(User);
            stopRingBackTone();
        }
        break;
    ...
    case Circular: {
        ...
        if (key == hangUpKey) {
            ...
            addSubscriber(key);
        else if (pressedKey == key && keyMode == NoFixed && !emptyDynamicStoreFlag)
            pushToTalk();
        else if (pressedKey == key && keyMode == Fixed) {
            hangUp(User);
            clearDynamicStore();
                ...
                osMutexRelease ( mutexState_id);
                    ...
                    if (key == var.n) {
                        if (!numberPudKey) {
                            ...
                            GPIO::getInstance()->configLed(numberPudKey);
...
//                stopRingTone();
                ...
                GPIO::getInstance()->configLed(assignedKey);
        ...
        case STATE_CREATE_RTP:
            if (incomingCallType == Telephone && key == Hash)
                ...
                hangUp();
            else*/ if (assignedKey == key && incomingCallType == Conference && keyMode == NoFixed) {
                    pushToTalk();
        ...
        case STATE_RECV_UDP:
            if (key == hangUpKey) {}
                ...
                assignedKey = pressedKey = Asterisk;
                    ...
                    if ((pressedKey = getKeysRingBuffData(Key, i)) == key) {
                        setMode(getKeysRingBuffData(Function, i), i);
                        break;
                    }
                }
            break;
...
//        jsonDocCapacity_ = JSON_OBJECT_SIZE(5) + JSON_ARRAY_SIZE(10);
//    else if (incomingCall) jsonDocCapacity_ = JSON_OBJECT_SIZE(6) + JSON_ARRAY_SIZE(1);
...
//    osDelay(50);
//    if (emptyDynamicStoreFlag) hangUp();
}
...
void Timer2_Callback(void const *arg)
{
    ...
    if (Call_Control::getInstance()->noFixedCount < 7) {
        ...
        GPIO::getInstance()->configLed(Call_Control::getInstance()->numberPudKey, false);
...
void Timer3_Callback(void const *arg)
{
    ...
    if ( !Call_Control::getInstance()->emptyDynamicStoreFlag&& sizeDynamicStore > 2) {
        ...
        GPIO::getInstance()->configLed( Call_Control::Asterisk, true, 300, 300);
    } else {
        GPIO::getInstance()->configLed( Call_Control::Asterisk, false);

        Call_Control::getInstance()->setCallType (Call_Control::Direct);
...
void Call_Control::setMode(uint8_t func, /*uint8_t control,*/ uint16_t i)
{
    setCallType(func);
//    noFixedKey = pressedKey;
    ...
    if (func == Group || func == Direct || func == Telephone) {
        osMutexWait(mutexKeyBuffer_id, osWaitForever);
        foundKey = Json::getInstance()->thisStation.keysRingBuffer[i];
        osMutexRelease(mutexKeyBuffer_id);
    ...
    GPIO::getInstance()->configLed(pressedKey);
