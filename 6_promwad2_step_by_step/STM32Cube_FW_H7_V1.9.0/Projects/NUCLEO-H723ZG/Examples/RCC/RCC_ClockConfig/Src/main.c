...
int main(void)
{
  ...
  /* Enable HSI oscillator and configure the PLL to reach the max system frequency (520 MHz)
...
static void SwitchSystemClock(void)
{
  if (__HAL_RCC_GET_PLL_OSCSOURCE() == RCC_PLLSOURCE_HSI)
  {
    ...
    /* Set SYSCLK frequency to 520000000 Hz, coming from the PLL which is clocked by CSI  */
    SystemClockCSI_Config();
  }
  else if (__HAL_RCC_GET_PLL_OSCSOURCE() == RCC_PLLSOURCE_HSE)
  {
    /* PLL source is HSE Bypass oscillator */
    /* Set SYSCLK frequency to 520000000 Hz, coming from the PLL which is clocked by HSI */
    SystemClockHSI_Config();
  }
  else if (__HAL_RCC_GET_PLL_OSCSOURCE() == RCC_PLLSOURCE_CSI)
  {
    /* PLL source is CSI oscillator */
    /* Set SYSCLK frequency to 520000000 Hz, coming from the PLL which is clocked by HSE Bypass */
