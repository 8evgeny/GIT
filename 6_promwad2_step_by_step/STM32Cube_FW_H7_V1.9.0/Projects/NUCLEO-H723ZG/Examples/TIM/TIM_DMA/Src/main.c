...
int main(void)
{
 /* This sample code shows how to use DMA with TIM1 Update request to transfer
  ...
  /* ---------------------------------------------------------------------------
  TIM1 input clock (TIM1CLK) is set to APB1 clock (PCLK1)x2, since APB1
  prescaler is 2.
    TIM1CLK = PCLK1x2
    PCLK1 = HCLK
    => TIM1CLK = HCLK = SystemCoreClock / 2 since AHB prescaler is 2 
  ...
  signal with a frequency equal to 17.57 KHz:
     - TIM1_Period = (SystemCoreClock/2 / 17570) - 1
