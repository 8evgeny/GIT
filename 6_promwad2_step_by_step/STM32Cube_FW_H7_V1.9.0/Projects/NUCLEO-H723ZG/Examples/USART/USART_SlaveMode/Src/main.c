...
int main(void)
{
  ...
  UsartHandle.Init.ClockPrescaler  = USART_PRESCALER_DIV1;
  ...
  HAL_USARTEx_EnableSlaveMode(&UsartHandle);

  /* Configure the Slave Select input pin */
  HAL_USARTEx_ConfigNSS(&UsartHandle, USART_NSS_SOFT);

  /* Disable the FIFO mode */
  HAL_USARTEx_DisableFifoMode(&UsartHandle);
