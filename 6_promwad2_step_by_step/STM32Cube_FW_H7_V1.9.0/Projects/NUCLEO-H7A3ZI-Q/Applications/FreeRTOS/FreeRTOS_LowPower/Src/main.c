...
static void QueueReceiveThread(void *argument);
...
int main(void)
{
  ...
  osqueueHandle = osMessageQueueNew (1U, sizeof(uint32_t), NULL);
  ...
  attr.name = "TxThread";
  TxThreadHandle = osThreadNew(QueueSendThread, NULL, (const osThreadAttr_t *)&attr);
  /* definition and creation of RxThread */
  attr.name = "RxThread";
  RxThreadHandle = osThreadNew(QueueReceiveThread, NULL, (const osThreadAttr_t *)&attr);
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIOA pins as analog */
