...
static void ADC_Config(void)
{
  ...
  AdcHandle_master.Init.ClockPrescaler        = ADC_CLOCK_ASYNC_DIV2;          /* Asynchronous clock mode, input ADC clock not divided */
...
/**
  ...
  *         (pin PA.04) and ADC channel (pin PC.03) to run this example.
...
static void WaveformVoltageGenerationForTest(void)
{
  ...
  if (HAL_DAC_ConfigChannel(&DacForWaveformTestHandle, &sConfig, DACx_CHANNEL_TO_ADCx_CHANNELa) != HAL_OK)
  {
    /* Channel configuration error */
    Error_Handler();
  }
  ...
  if (HAL_DAC_Start_DMA(&DacForWaveformTestHandle, DACx_CHANNEL_TO_ADCx_CHANNELa, (uint32_t *)Waveform_8bits, WAVEFORM_TEST_SAMPLES_NUMBER, DAC_ALIGN_8B_R) != HAL_OK)
  {
    /* Start DMA Error */
    Error_Handler();
  }
