...
int main(void)
{
  ...
  /* Enable HSE oscillator and configure the PLL to reach the max system frequency (280 MHz)
...
static void SwitchSystemClock(void)
{
  if (__HAL_RCC_GET_PLL_OSCSOURCE() == RCC_PLLSOURCE_HSI)
  {
    ...
    /* Set SYSCLK frequency to 280000000 Hz, coming from the PLL which is clocked by CSI  */
    SystemClockCSI_Config();
  }
  else if (__HAL_RCC_GET_PLL_OSCSOURCE() == RCC_PLLSOURCE_HSE)
  {
    /* PLL source is HSE Bypass oscillator */
    /* Set SYSCLK frequency to 280000000 Hz, coming from the PLL which is clocked by HSI */
    SystemClockHSI_Config();
  }
  else if (__HAL_RCC_GET_PLL_OSCSOURCE() == RCC_PLLSOURCE_CSI)
  {
    /* PLL source is CSI oscillator */
    /* Set SYSCLK frequency to 280000000 Hz, coming from the PLL which is clocked by HSE Bypass */
