...
static uint8_t Jpeg_CallbackFunction(uint8_t* Row, uint32_t DataLength)
{
  ...
  offset = (LCD_LAYER_0_ADDRESS + (IMAGE_WIDTH * line_counter * 4));
  ...
  for(counter = 0; counter < IMAGE_WIDTH; counter++)
  {
    ...
    *(__IO uint32_t *)(LCD_LAYER_0_ADDRESS + (counter*4) + (IMAGE_WIDTH * line_counter  * 4)) = ARGB32Buffer[counter];
  }
#endif
  ...
  {
    pixel = *(__IO uint32_t *)(LCD_LAYER_0_ADDRESS + (width_counter*4) + (IMAGE_WIDTH * line_counter * 4));
    result1 = (((pixel & 0x00FF0000) >> 16) | ((pixel & 0x000000FF) << 16));
    pixel = pixel & 0xFF00FF00;
    result = (result1 | pixel);
    *(__IO uint32_t *)(LCD_LAYER_0_ADDRESS + (width_counter*4) + (IMAGE_WIDTH *  line_counter  * 4)) = result;
  }
#endif
