...
#ifdef MX_WIFI_CONF_H
        if (WIFI_Connect((char*) ssid_ascii, password_ascii, NET_WIFI_SM_AUTO) != NET_OK)
#else
...
int NET_Init(void)
{
  if (NET_OK != net_if_init(&netif, mx_wifi_driver, &net_handler))
  {
    return -1;
  }

  if (NET_OK != net_if_wait_state(&netif,NET_STATE_INITIALIZED,STATE_TRANSITION_TIMEOUT))
  {
    return -1;
  }

  if ( NET_OK != net_if_start (&netif) )
  {
    return -1;
  }

  if ( NET_OK != net_if_wait_state(&netif,NET_STATE_READY,STATE_TRANSITION_TIMEOUT))
  {
    return -1;
  }
...
{  
  ...
  {
    if(NET_OK == net_if_connect (&netif) )
    {
      if ( NET_OK == net_if_wait_state(&netif,NET_STATE_CONNECTED,STATE_TRANSITION_TIMEOUT))
      {
        return NET_OK;
      }
      else
...
int Get_Date_Time(char* date_time)
{
  ...
  char  DATE_SERVER [] = "www.worldtimeapi.org";
  ...
  ServerIp.sin_len = sizeof(struct net_sockaddr_in);
