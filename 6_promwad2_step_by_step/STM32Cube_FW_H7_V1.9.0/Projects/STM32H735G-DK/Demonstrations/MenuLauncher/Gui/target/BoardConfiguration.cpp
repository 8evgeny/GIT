...
void hw_init()
{
    unsigned long __IO demo_addr = 0x0;
    FLASH_OBProgramInitTypeDef FLASH_OBInitStruct;


    HAL_PWR_EnableBkUpAccess();
    demo_addr = READ_REG(BKP_REG_DEMO_LOC);
    WRITE_REG(BKP_REG_DEMO_LOC, 0);
    HAL_PWR_DisableBkUpAccess();

    HAL_Init();

    /* Check and update the Flash Boost option byte if required to allow VOS0 with 550MHz system clock */
    /* Get Option Bytes */
    HAL_FLASHEx_OBGetConfig(&FLASH_OBInitStruct); 
    if (FLASH_OBInitStruct.FreqBoostState != OB_CPUFREQ_BOOST_ENABLE)
    {
      ...
      FLASH_OBInitStruct.OptionType     = OPTIONBYTE_FREQ_BOOST; 
      FLASH_OBInitStruct.FreqBoostState = OB_CPUFREQ_BOOST_ENABLE;
    ...
    if(demo_addr)
    {
        ...
        pFunction JumpToApplication = (pFunction) (*(__IO uint32_t*) (demo_addr + 4));
        __set_MSP(*(__IO uint32_t*) demo_addr);
        JumpToApplication();
