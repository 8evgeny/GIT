...
#define DC_RES          1
#define NOISE_RES       1
#define INPUT_FREQUENCY 250000000                             // TIM6 counter clock (prescaled APB1)
...
static uint32_t DAC_NoiseWaveConfig(uint16_t outputAmplitude)
{
  ...
  HAL_DAC_SetValue(&DacHandle, DACx_CHANNEL, DAC_ALIGN_12B_R, NoiseWave[0]);
