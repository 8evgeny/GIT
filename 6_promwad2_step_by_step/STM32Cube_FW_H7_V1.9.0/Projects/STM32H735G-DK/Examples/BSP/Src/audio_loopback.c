...
#pragma location=0x38003000
uint16_t recordPDMBuf[AUDIO_IN_PDM_BUFFER_SIZE];
#elif defined ( __CC_ARM )
__attribute__((at(0x38003000))) uint16_t recordPDMBuf[AUDIO_IN_PDM_BUFFER_SIZE];
#elif defined ( __GNUC__ )
__attribute__((section(".RAM_D3"))) uint16_t recordPDMBuf[AUDIO_IN_PDM_BUFFER_SIZE];
#endif
...
uint32_t VolumeLevel = 80, AudioOutState, AudioInState;
...
uint8_t* Audio_Out_State[4] = {
  (uint8_t*)" AUDIO_OUT_STATE_RESET  ",
  (uint8_t*)" AUDIO_OUT_STATE_PLAYING",
  (uint8_t*)" AUDIO_OUT_STATE_STOP   ",
  (uint8_t*)" AUDIO_OUT_STATE_PAUSE  "
};

uint8_t* Audio_In_State[4] = {
  (uint8_t*)"AUDIO_IN_STATE_RESET    ",
  (uint8_t*)"AUDIO_IN_STATE_RECORDING",
  (uint8_t*)"AUDIO_IN_STATE_STOP     ",
  (uint8_t*)"AUDIO_IN_STATE_PAUSE    "
};

uint8_t* Mute_State[2] = {
  (uint8_t*)" BSP_AUDIO_MUTE_DISABLED",
  (uint8_t*)" BSP_AUDIO_MUTE_ENABLED ",
};
