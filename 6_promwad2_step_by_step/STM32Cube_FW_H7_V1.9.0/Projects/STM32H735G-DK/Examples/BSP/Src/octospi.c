...
#define QPI_START_ADDRESS   7*MX25LM51245G_SECTOR_64K
...
uint8_t* InterfaceModeText[2] =
{
  (uint8_t*) "OSPI_SPI_MODE",
  (uint8_t*) "OSPI_OPI_MODE"
};
...
static void OSPI_STR_Mode(void)
{
  ...
  for(j = 0; j < 2; j++)
  {
    ...
    {
      if(BSP_OSPI_NOR_Erase_Block(0, QPI_START_ADDRESS + i*block_size, MX25LM51245G_ERASE_64K) != BSP_ERROR_NONE)
      {
    ...
    {
      if(BSP_OSPI_NOR_Write(0, qspi_aTxBuffer, QPI_START_ADDRESS + i*BUFFER_SIZE, BUFFER_SIZE) != BSP_ERROR_NONE)
      {
      ...
      if(BSP_OSPI_NOR_Read(0, qspi_aRxBuffer, QPI_START_ADDRESS + i*BUFFER_SIZE, BUFFER_SIZE) != BSP_ERROR_NONE)
      {
...
static void OSPI_STR_MM_Mode(void)
{
  ...
  for(j = 0; j < 2; j++)
  {
    ...
    QSPI_MMP_pointer = (uint8_t*)(0x90000000 + QPI_START_ADDRESS);
    for(i = 0; i < (temp_value/BUFFER_SIZE); i++)
    {
...
static void OSPI_DTR_Mode(void)
{
  ...
  for(i = 0; i < temp_value/block_size; i++)
  {
    if(BSP_OSPI_NOR_Erase_Block(0, i*block_size, MX25LM51245G_ERASE_64K) != BSP_ERROR_NONE)
    {
      UTIL_LCD_DisplayStringAt(Xpos, Ypos, (uint8_t*)"Erase Failed.", LEFT_MODE);
    }
    while(BSP_OSPI_NOR_GetStatus(0) == BSP_ERROR_BUSY);
  ...
  for(i = 0; i < (temp_value/BUFFER_SIZE); i++)
  {
    if(BSP_OSPI_NOR_Write(0, qspi_aTxBuffer, i*BUFFER_SIZE, BUFFER_SIZE) != BSP_ERROR_NONE)
    {
      UTIL_LCD_DisplayStringAt(Xpos, Ypos, (uint8_t*)"WRITE : FAILED.", LEFT_MODE);
      NEXT_LINE
        UTIL_LCD_DisplayStringAt(Xpos, Ypos, (uint8_t*)"Test Aborted.", LEFT_MODE);
    }
    if(BSP_OSPI_NOR_Read(0, qspi_aRxBuffer, i*BUFFER_SIZE, BUFFER_SIZE) != BSP_ERROR_NONE)
    {
      UTIL_LCD_DisplayStringAt(Xpos, Ypos, (uint8_t*)"Read : FAILED.", LEFT_MODE);
      NEXT_LINE
        UTIL_LCD_DisplayStringAt(Xpos, Ypos, (uint8_t*)"Test Aborted.", LEFT_MODE);
...
static void OSPI_SetHint(void)
{
  ...
  UTIL_LCD_DisplayStringAt(0, 0, (uint8_t*)"OSPI Flash", CENTER_MODE);
  UTIL_LCD_SetFont(&Font12);
  UTIL_LCD_DisplayStringAt(0, 30, (uint8_t*)"This example shows how to write", CENTER_MODE);
  UTIL_LCD_DisplayStringAt(0, 45, (uint8_t*)"and read data on OSPI Flash memory", CENTER_MODE);
