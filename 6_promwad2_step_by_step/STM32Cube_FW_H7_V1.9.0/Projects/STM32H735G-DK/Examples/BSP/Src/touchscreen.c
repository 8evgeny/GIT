...
static uint32_t Touchscreen_Handle_NewTouch(void)
{
  ...
  if(TS_MTState.TouchDetected)
  {
    ...
    if(drawTouch1 == 1)
    {
      ...
      sprintf((char*)lcd_string, "x1 = %d, y1 = %d, Event = %s, Weight = %ld",
              x1,
              y1,
    ...
    if(TS_MTState.TouchDetected > 1)
    {
      ...
      if(drawTouch2 == 1)
      {
        sprintf((char*)lcd_string, "x2 = %d, y2 = %d, Event = %s, Weight = %ld",
                x2,
                y2,
...
void Touchscreen_demo3(void)
{
  ...
  {
    for(l = 0; l < hTS.Height/34; l++)
    {
      UTIL_LCD_DrawRect(40*k, 34*l,40,34, UTIL_LCD_COLOR_BLACK);
    }
  }
...
static void TS_Update(void)
{
  ...
  if(TS_State.TouchDetected)
  {
    ...
    {
      for(j = 0; j < hTS.Height/34; j++)
      {
        if(((x_new_pos > 40*i) && (x_new_pos < 40*(i+1))) && ((y_new_pos > 34*j) && (y_new_pos < 34*(j+1))))
        {
          UTIL_LCD_FillRect(x_previous_pos, y_previous_pos,40,34, UTIL_LCD_COLOR_WHITE);
          UTIL_LCD_DrawRect(x_previous_pos, y_previous_pos,40,34, UTIL_LCD_COLOR_BLACK);

          UTIL_LCD_FillRect(40*i, 34*j,40,34, colors[(touchscreen_color_idx++ % 24)]);
