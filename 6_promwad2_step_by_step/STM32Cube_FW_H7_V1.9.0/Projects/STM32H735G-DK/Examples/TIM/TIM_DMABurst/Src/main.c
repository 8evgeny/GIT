/**
  ...
  * @brief   This sample code shows how to use DMA with TIM5 Update request to
  *          transfer Data from memory to TIM5 Capture Compare Register 3
...
/*##-1- Configure the TIM peripheral #######################################*/ 
  /* -----------------------------------------------------------------------
    TIM5 Configuration: generate 1 PWM signal using the DMA burst mode:
   
    TIM5 input clock (TIM5CLK) is set to APB1 clock (PCLK1) x 2, since APB1
    prescaler is 2.
    TIM1CLK = 2 x PCLK1
    PCLK1 = HCLK/2
    => TIM5CLK = HCLK = SystemCoreClock / 2 since AHB prescaler is 2.
    
    To get TIM5 counter clock at 10 MHz, the prescaler is computed as follows:
    ...
    TIM5 Channel1 duty cycle = (TIM5_CCR1/ TIM5_ARR)* 100 = 33.33%
  ...
  TimHandle.Init.Prescaler         = ((SystemCoreClock/2) / (10 * 1000000)) - 1;
