/**
  ******************************************************************************
  * @file    FatFs/FatFs_USBDisk_Standalone/Src/usbh_conf.c
...
void HAL_HCD_MspInit(HCD_HandleTypeDef *hhcd)
{
  ...
  else if(hhcd->Instance == USB1_OTG_HS)
  {
    ...
    /* Initialize the IO */
    io_init_structure.Pin  =IO_PIN_9;
    io_init_structure.Pull = IO_PULLUP;
    io_init_structure.Mode = IO_MODE_OUTPUT_PP;
...
USBH_StatusTypeDef USBH_LL_DriverVBUS(USBH_HandleTypeDef *phost, uint8_t state)
{
  ...
  {
    /* Configure Low Charge pump */
    BSP_IO_WritePin(0, IO_PIN_9,IO_PIN_RESET);
  }
  else
  {
    /* Drive High Charge pump */
   BSP_IO_WritePin(0, IO_PIN_9,IO_PIN_SET);
  }
#endif  
  HAL_Delay(200);
  return USBH_OK;  
