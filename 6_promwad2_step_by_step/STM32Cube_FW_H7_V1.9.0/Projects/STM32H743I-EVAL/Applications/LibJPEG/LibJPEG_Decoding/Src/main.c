...
static uint8_t Jpeg_CallbackFunction(uint8_t* Row, uint32_t DataLength)
{
  ...
  offset = (LCD_FB_START_ADDRESS + (IMAGE_WIDTH * (IMAGE_HEIGHT - line_counter - 1) * 4));
...
/* Configure the DMA2D Mode, Color Mode and output offset */
  ...
  for(counter = 0; counter < IMAGE_WIDTH; counter++)
  {
    ...
    *(__IO uint32_t *)(LCD_FB_START_ADDRESS + (counter*4) + (IMAGE_WIDTH * (IMAGE_HEIGHT - line_counter - 1) * 4)) = ARGB32Buffer[counter];
  }  
  ...
  uint32_t pixel = 0, width_counter, result = 0, result1 = 0;
   ...
   {
     pixel = *(__IO uint32_t *)(LCD_FB_START_ADDRESS + (width_counter*4) + (IMAGE_WIDTH * (IMAGE_HEIGHT - line_counter - 1) * 4)); 
     result1 = (((pixel & 0x00FF0000) >> 16) | ((pixel & 0x000000FF) << 16));
     pixel = pixel & 0xFF00FF00;
     result = (result1 | pixel);
     *(__IO uint32_t *)(LCD_FB_START_ADDRESS + (width_counter*4) + (IMAGE_WIDTH * (IMAGE_HEIGHT - line_counter - 1) * 4)) = result;
