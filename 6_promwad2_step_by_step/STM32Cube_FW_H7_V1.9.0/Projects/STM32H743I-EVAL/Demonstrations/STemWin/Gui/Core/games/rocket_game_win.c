...
static SPRITE CoinSprite = { 644, 150, -8, -6, GUI_COUNTOF(CoinAnimationImages), CoinAnimationImages, 0 };

/* Moving objects array */
SPRITE ObstaclesArray[] =
{
  { 644, 200, -6, -6, GUI_COUNTOF(MoonImages), MoonImages, 0 },
  { 644, 250, -8, -8, GUI_COUNTOF(Planet1Images), Planet1Images, 0 },
  { 644, 300, -10, -10, GUI_COUNTOF(Planet2Images), Planet2Images, 0 },
  { 644, 350, -12, -12, GUI_COUNTOF(Planet3Images), Planet3Images, 0 },
};
...
static void _OnPaint_MenuButton(BUTTON_Handle hObj)
{
  ...
  {
    GUI_DrawBitmap(&bmmain_menu, 0, 0);
  }
  else
  {
    GUI_DrawBitmap(&bmmain_menu, 0, 0);
...
static void OnGameTimer(GUI_TIMER_MESSAGE * pTM)
{
        ...
        ObstaclesArray[ObstacleId].yPos + ObstaclesArray[ObstacleId].apImages[0]->YSize <= rocketCollisionArea.y0))
  {
    ...
    ObstaclesArray[ObstacleId].xPos = 644;
  ...
  else
  {
    ...
    if (ObstaclesArray[ObstacleId].xPos >= 644)
    {
      ChangeObstacleId();
    }
...
static void _cbWinPlayScreen(WM_MESSAGE * pMsg)
{
  ...
  {
    case WM_CREATE:
      hItem = BUTTON_CreateEx(256, 332 - 80, bmplay_button.XSize, bmplay_button.YSize, pMsg->hWin, WM_CF_SHOW, 0, ID_BUTTON_PLAY);
      WM_SetCallback(hItem, _cbButton_Play); 

      break;

    case WM_PAINT:
      GUI_DrawBitmap(&bmgame_title, 222, 0);
      break;
...
static void _cbWinGameoverScreen(WM_MESSAGE * pMsg)
{
  ...
  {
    case WM_CREATE:
      hItem = BUTTON_CreateEx(256, 332 - 80, bmreplay_button.XSize, bmreplay_button.YSize, pMsg->hWin, WM_CF_SHOW, 0, ID_BUTTON_REPLAY);
      WM_SetCallback(hItem, _cbButton_Replay); 
      break;

    case WM_PAINT:
      GUI_DrawBitmap(&bmgame_over,              182,          148 - 80);
      GUI_DrawBitmap(&bmrocket_down_game_over,  290,          110 - 80);
      GUI_DrawBitmap(&bmrocket_down1_game_over, 550,          354 - 80);
      GUI_DrawBitmap(&bmrocket_down2_game_over, 56,           200 - 80);
      GUI_DrawBitmap(&bmscore_shape_game_over,  250,          270 - 80);
      GUI_DrawBitmap(&bmscore,                  250 + 8,      270 + 10 - 80);
      GUI_DrawBitmap(&bmgoldCoin,               250 + 8 + 75, 270 + 3 - 80);
      ...
      GUI_DispStringAt(tmp, 250 + 8 + 80 + 30 , 270 + 7 - 80);
...
static void _cbWinParallax(WM_MESSAGE * pMsg)
{
  ...
  switch (pMsg->MsgId)
  {
    case WM_CREATE:
      ...
      HwinMainMenuButton = BUTTON_CreateEx(640 - bmmain_menu.XSize - 2, 2, bmmain_menu.XSize, bmmain_menu.YSize, pMsg->hWin, WM_CF_SHOW | WM_CF_STAYONTOP, 0, ID_BUTTON_MENU);
...
static void Startup(WM_HWIN hWin, uint16_t xpos, uint16_t ypos)
{
  ...
  hWinBkc = WM_CreateWindowAsChild(0, 0, 640, 480, hWin, WM_CF_SHOW, _cbWinParallax, 0);

  /* Create the rocket window */
  hWinRocket = WM_CreateWindowAsChild(264, 192, 100, 61, hWinBkc, WM_CF_HASTRANS | WM_CF_SHOW | WM_CF_MOTION_Y | WM_CF_MOTION_X, _cbWinRocket, 0);

  /* Create the play window */
  hWinPlayScreen = WM_CreateWindowAsChild(0, 80, 640, 400, hWinBkc, WM_CF_HASTRANS | WM_CF_SHOW, _cbWinPlayScreen, 0);

  /* Create the replay window */
  hWinGameoverScreen = WM_CreateWindowAsChild(0, 80, 640, 400, hWinBkc, WM_CF_HASTRANS, _cbWinGameoverScreen, 0);
