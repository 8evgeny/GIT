...
#define ATTRIBUTE_SDIO_HEAP_LOCATION __attribute__ ((section ("SDIOSection"))) __attribute__ ((aligned(4)))
#elif defined __ICCARM__
#define PRAGMA_SDIO_HEAP_LOCATION _Pragma("location=\"SDIOSection\"")
  ...
  for( ;; )
  {
    ...
    if( event.status == osEventMessage )
    {
            ...
            if(BSP_SD_DetectITConfig(0) == BSP_ERROR_NONE)
            {
                if(FATFS_LinkDriver((Diskio_drvTypeDef*)MSDDISK_Driver, (TCHAR*)MSDDISK_Drive) == 0)
                {
                    if (!f_mount(&MSDDISK_FatFs,MSDDISK_Drive,  0))
                    {
                        ...
                        STORAGE_NotifyConnectionChange (MSD_DISK_UNIT, event.value.v);
                    }
                }
            }
        }
      ...
      case MSDDISK_DISCONNECTION_EVENT:
        {
            uint32_t timer = osKernelSysTick() + 200;
            StorageStatus[MSD_DISK_UNIT] = 0;
            STORAGE_NotifyConnectionChange (MSD_DISK_UNIT, event.value.v);

            /* first ensure the SDCard is ready for a next operation */
