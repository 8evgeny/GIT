    ...
    onRemove(this, &AudioPlayerPlaylist::removeElement)
{
    ...
    background.setColor(Color::getColorFrom24BitRGB(0x20, 0x28, 0x2B));
    add(background);
...
void AudioPlayerPlaylistElement::setCover(Bitmap &bitmap)
{
    ...
    {
        const uint8_t *src = bitmap.getData() + i * bpp * bitmap.getWidth() * 3; //every fourth line
        for (int j = 0; j < cover.getWidth(); j++)
        {
            ...
            *dst++ = *src++;
            ...
            if (coverBmp.getFormat() == Bitmap::ARGB8888)
                dst++;
            src += 3 * bpp; //skip 3 pixels as we are scaling down            
        }
    }
}
