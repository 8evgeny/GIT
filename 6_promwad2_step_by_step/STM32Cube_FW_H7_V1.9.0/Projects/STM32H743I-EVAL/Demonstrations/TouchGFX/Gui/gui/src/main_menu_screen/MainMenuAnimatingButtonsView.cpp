...
void MainMenuAnimatingButtonsView::setupScreen()
{
    ...
    demoIDs[2] = Defines::KNIGHT_GAME;
    icons[2].setBitmap(Bitmap(BITMAP_ANIMATING_BUTTONS_MENU_ICONS_KNIGHT_TINY_ID));
    texts[2].setTypedText(TypedText(T_ANIMATING_BUTTON_GAME_KNIGHT_HEADLINE));
    ...
    demoIDs[7] = Defines::BUMP_MAP;
    icons[7].setBitmap(Bitmap(BITMAP_ANIMATING_BUTTONS_MENU_ICONS_BUMP_MAP_TINY_ID));
    texts[7].setTypedText(TypedText(T_ANIMATING_BUTTON_BUMP_MAP_HEADLINE));
...
void MainMenuAnimatingButtonsView::StartupAnimationBoxDoneHandler(const ZoomAnimationBox& box)
{
    if (currentAnimationState == ANIMATE_BOX_IN)
    {
        ...
        for (int i = 0; i < NUMBER_OF_BUTTONS; i++)
        {
            ...
            {
                buttonContainers[i].startMoveAnimation(64, 130 + (buttonContainers[i].getHeight() * i) + (i)*BUTTONS_SPACE, duration, EasingEquations::backEaseOut, EasingEquations::backEaseOut);
            }
            else
            {
                buttonContainers[i].startMoveAnimation(HAL::DISPLAY_WIDTH - buttonContainers[i].getWidth() - 64, 130 + (buttonContainers[i].getHeight() * (i-numbersToTheLeft)) + (i-numbersToTheLeft)*BUTTONS_SPACE, duration, EasingEquations::backEaseOut, EasingEquations::backEaseOut);
            }
