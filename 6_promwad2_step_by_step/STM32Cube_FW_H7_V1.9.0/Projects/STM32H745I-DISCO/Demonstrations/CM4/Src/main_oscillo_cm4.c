/**
  ******************************************************************************
  * @file    Demonstration/STM32H745-Discovery_Demo/CM4/Src/main.c
...
uint16_t   Samples[SAMPLES_BUFFER_SIZE*2];
...
int main(void)
{
  ...
  */
  ...
  while (1)
  {
    /* Check if there is request to enter in stand by mode */
    if( *(uint32_t*)0x38000000 == 0xDEAD)
    {
      *(uint32_t*)0x38000000 = 0x12345678; 
...
void TIM_Reconfig()
{
  ...
  break;
  case 10:   {Period = 19;     Prescalar =999;}        // Timer update rate = ADC conversion Frequency = 10 KHz   
  break;
  case 100:  {Period = 1;      Prescalar =999;}        // Timer update rate = ADC conversion Frequency = 100 KHz
  break;
  case 1000: {Period = 1;      Prescalar =99;}         // Timer update rate = ADC conversion Frequency = 1 MHz
  break;  
  case 2000: {Period = 1;      Prescalar =49;}         // Timer update rate = ADC conversion Frequency = 2 MHz
  break;
  case 4000: {Period = 25;      Prescalar =1;}         // Timer update rate = ADC conversion Frequency = 4 MHz   
  break;
  default:   {Period = 25 ;     Prescalar =1;}         // Sinon ADC frequency = ADC Frequency MAX
  break;
...
static void Samples_Send(void)
{
  ...
  { 
    for(i=0;  i <  (ADAPT_SAMPLES_BUFFER_SIZE(Conversion_Frequency)/2);  i++)
    {
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *UartHandle)
{
    ...
    TransmitSuspend = 1;
    if (strncmp((char *)Received_Command,"8 Msample\n",Received_Command_Size)==0)
    {
...
static void DMA_Config(void)
{
  ...
  {  
    ...
    HAL_NVIC_SetPriority(ADCx_DMA_IRQn, 11,0);
