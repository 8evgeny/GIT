...
After a system start-up, LED1 is ON to indicate that the CM7 is in RUN mode
and LED2 is ON to indicate that the CM4 is in RUN mode.
...
I2C transmission by using the second board.
...
LED1 will turn ON if an error occurred during reception or transmission process.
...
LED1 will turn ON if an error occurred.
...
SCL Pin: PD.12 (pin D15 on CN2)
SDA Pin: PD.13 (pin D14 on CN2)
...
How to run:
  ...
  => If the test passed, then LED1 on the second board will toggle each 100 ms to indicate
     ...
     If an error occurred during the communication LED1 of the second board will turn On.
...
The CPU is only used to intercept a DMA transfer error if any and sets the LED1 to ON in this case
...
@par Hardware and Software environment
  ...
  - STM32H745I-Discovery set-up:
    - Connect a wire between 1st board PD.12 pin (SCL pin D15 on CN2) and
      2nd board PD.12 pin (SCL pin D15 on CN2).
    - Connect a wire between 1st board PB.13 pin (SDA pin D14 on CN2) and
      2nd board PD.13 pin (SDA pin D14 on CN2).
