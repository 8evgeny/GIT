...
@note when the define USE_VOS0_480MHZ_OVERCLOCK (in main.h) is set to one, user can switch between VOS0 at 480MHz and VOS1 at 400MHz
  ...
  !!!Attention!!!
   ...
   Note that the Board HW configuration must match with the FW configuration
...
The check could be done using this macro : __HAL_RCC_GET_FLAG(RCC_FLAG_D2CKRDY).
  ...
  STM32H747I-EVAL Eval board's LEDs can be used to monitor the application status:
...
In order to make the program work, you must do the following :
 ...
 - For CM4 target configuration (STM32H747I_EVAL_CM4) :
     - Rebuild all files
     - Load image into target memory

 - For CM7 target configuration  :
  - Select FPU On (STM32H747I_EVAL_CM7_FPU-On) or FPU Off (STM32H747I_EVAL_CM7_FPU-Off) workspace
