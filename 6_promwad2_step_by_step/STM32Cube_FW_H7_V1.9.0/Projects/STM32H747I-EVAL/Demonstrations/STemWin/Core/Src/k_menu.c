...
static const BITMAP_ITEM _aBitmapItem[] = {
  ...
  {&bmDual_core_idle_210x210,        &bmDual_core_clicked_210x210,       "DUAL CORE"          , 5 },
};
          ...
          switch (Id)
          {
            case ID_BUTTON_MODULE:
              ...
              if( (module_prop[current_module_id].module->id == MODULE_AUDIO_PLAYER) \
               ...
               || (module_prop[current_module_id].module->id == MODULE_DUAL_CORE) )
              {
...
void k_InitMenu(void) 
{
  ...
  hItem = BUTTON_CreateEx(115, 300, 210, 210, WM_HBKWIN, WM_CF_SHOW | WM_CF_HASTRANS, 0 , ID_BUTTON_MODULE0);
  WM_SetCallback(hItem, _cbButton_ok);

  hItem = BUTTON_CreateEx(486, 300, 210, 210, WM_HBKWIN, WM_CF_SHOW | WM_CF_HASTRANS, 0 , ID_BUTTON_MODULE1);
  WM_SetCallback(hItem, _cbButton_ok);
  
  hItem = BUTTON_CreateEx(115, 200, 210, 210, WM_HBKWIN, WM_CF_SHOW | WM_CF_HASTRANS, 0 , ID_BUTTON_MODULE2);
  WM_SetCallback(hItem, _cbButton_ok);

  hItem = BUTTON_CreateEx(486, 200, 210, 210, WM_HBKWIN, WM_CF_SHOW | WM_CF_HASTRANS, 0 , ID_BUTTON_MODULE3);
  WM_SetCallback(hItem, _cbButton_ok);

  hItem = BUTTON_CreateEx(370, 130, 210, 210, WM_HBKWIN, WM_CF_SHOW | WM_CF_HASTRANS, 0 , ID_BUTTON_MODULE4);
  WM_SetCallback(hItem, _cbButton_ok);
  
  hItem = BUTTON_CreateEx(720, 420, 75, 60, WM_HBKWIN, WM_CF_SHOW | WM_CF_HASTRANS, 0 , ID_BUTTON_INFO);
  WM_SetCallback(hItem, _cbButton_info);

  /* Create different buttons to be used */
  hItem = BUTTON_CreateEx(350, 220, 210, 260, WM_HBKWIN, WM_CF_SHOW | WM_CF_HASTRANS, 0 , ID_BUTTON_MODULE);
  WM_SetCallback(hItem, _cbButton_ok);
#if 0
  ...
  hItem = TEXT_CreateEx(490, 450, 225, 20, WM_HBKWIN, WM_CF_HIDE, TEXT_CF_LEFT, ID_TEXT_STORAGE, "Please insert media storage");
  TEXT_SetFont(hItem, GUI_FONT_20_1);
  TEXT_SetTextColor(hItem, GUI_WHITE);
}
