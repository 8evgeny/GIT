...
{
  { WINDOW_CreateIndirect,    "Audio Player",   ID_FRAMEWIN_INFO,       0, 0, 800, 480, 0, WM_CF_HASTRANS, 0 },
  { TEXT_CreateIndirect,      "",               ID_TITLE,               0,  288,  800,  48,  0, 0x0,  0 },
  { TEXT_CreateIndirect,      "",               ID_ELAPSED_TIME,        180, 457,  30,  22,  0, 0x0,  0 },
  { TEXT_CreateIndirect,      "",               ID_TOTAL_TIME,          604, 457,  30,  22,  0, 0x0,  0 },
};
      ...
      if(Id == ID_PROCESS_TIMER)
      {
        if(AUDIOPLAYER_GetState() == AUDIO_STATE_PLAY)
        {
          if(!PlayerSeekInProgress)
          {
            ...
            sprintf((char *)tmp , "%02d:%02d", (int )(progress/60), (int )(progress%60) );
        ...
        else if(AUDIOPLAYER_GetState() == AUDIO_STATE_EOF)
        {
          ...
          sprintf((char *)tmp , "%02d:%02d", (int )(duration/60), (int )(duration%60) );
          hItem = WM_GetDialogItem(hWinAudioPlayer, ID_ELAPSED_TIME);
          TEXT_SetText(hItem, tmp);
    ...
    if(AUDIOPLAYER_GetFileInfo(filename, &AudioInfo) == AUDIO_ERROR_NONE)
    {
      if(AUDIOPLAYER_SelectFile(filename) == AUDIO_ERROR_NONE)
      {
        ...
        if(AUDIOPLAYER_Play(AudioInfo.SampleRate) == AUDIO_ERROR_NONE)
        {
          ...
          TEXT_SetFont(WM_GetDialogItem(hWinAudioPlayer, ID_TITLE), &GUI_FontLubalGraph24B);
          TEXT_SetTextAlign(WM_GetDialogItem(hWinAudioPlayer, ID_TITLE), TEXT_CF_HCENTER | TEXT_CF_VCENTER);
...
{
  GUI_DrawBitmap(&bmPlaylistMenu, 15, 30);
}
...
static void _cbMainListDialog(WM_MESSAGE * pMsg) 
{
  ...
  {
    case WM_CREATE:  
      hItem = BUTTON_CreateEx(740, 5, 55, 70, pMsg->hWin, WM_CF_SHOW, 0, ID_PLAY_LIST_BACK_BUTTON);
...
static void _OnPaint_visualeq_button(BUTTON_Handle hObj) 
{
  if(VisEqClicked == 0)
  {
    GUI_DrawBitmap(&bmVisualUnselected, 15, 15);
  }
  else
  {
    GUI_DrawBitmap(&bmVisualSelected, 15, 15);
  }
}
...
{  
  GUI_DrawBitmap(&bmPlaylist, 15, 30);
}
...
static void _cbMainDialog(WM_MESSAGE * pMsg) 
{
  ...
  case WM_INIT_DIALOG:    
    ...
    /* Playlist */
    hItem = BUTTON_CreateEx(346, 10, 128, 46, pMsg->hWin, WM_CF_SHOW, 0, ID_PLAY_LIST_BUTTON);
    WM_SetCallback(hItem, _cbButton_play_list_button);  
    /* Shuffle */
    hItem = BUTTON_CreateEx(280, 14, 38, 60, pMsg->hWin, WM_CF_SHOW, 0, ID_SHUFFLE_BUTTON);
    WM_SetCallback(hItem, _cbButton_shuffle_button);    
    /* Repeat */
    hItem = BUTTON_CreateEx(498, 14, 42, 60, pMsg->hWin, WM_CF_SHOW, 0, ID_REPEAT_BUTTON);
    WM_SetCallback(hItem, _cbButton_repeat_button);
    /* Play */
    hItem = BUTTON_CreateEx(345, 348, 110, 110, pMsg->hWin, WM_CF_SHOW, 0, ID_PLAY_BUTTON);
    WM_SetCallback(hItem, _cbButton_play_button);
    /* Previous */
    hItem = BUTTON_CreateEx(175, 370, 61, 61, pMsg->hWin, WM_CF_SHOW, 0, ID_PREVIOUS_BUTTON);
    WM_SetCallback(hItem, _cbButton_previous_button);
    /* Next */
    hItem = BUTTON_CreateEx(565, 370, 61, 61, pMsg->hWin, WM_CF_SHOW, 0, ID_NEXT_BUTTON);
    WM_SetCallback(hItem, _cbButton_next_button);
    /* Stop Button */
    hItem = BUTTON_CreateEx(480, 370, 60, 60, pMsg->hWin, WM_CF_SHOW, 0, ID_STOP_BUTTON);
    WM_SetCallback(hItem, _cbButton_stop_button);
    /* Pause Button */
    hItem = BUTTON_CreateEx(260, 370, 60, 60, pMsg->hWin, WM_CF_SHOW, 0, ID_PAUSE_BUTTON);
    WM_SetCallback(hItem, _cbButton_pause_button);
    /* Visual equaliser */
    hItem = BUTTON_CreateEx(720, 385, 62, 60, pMsg->hWin, WM_CF_SHOW, 0, ID_VISUALEQ_BUTTON);
    WM_SetCallback(hItem, _cbButton_visualeq_button);
    /* Back Button */
    hItem = BUTTON_CreateEx(740, 5, 55, 70, pMsg->hWin, WM_CF_SHOW, 0, ID_BACK_BUTTON);
    WM_SetCallback(hItem, _cbButton_back_button);
    /* Slider Duration */
    hItem = SLIDER_CreateEx(218, 455, 382, 15, pMsg->hWin, WM_CF_SHOW, 0, ID_SLIDER_DURATION);
    SLIDER_SetValue(hItem, 0);
    SLIDER_SetWidth(hItem, 0);
    SLIDER_SetSTSkin(hItem);

    /* Volume slider */
    hItem = SLIDER_CreateEx(0, 338, 800, 15, pMsg->hWin, WM_CF_SHOW, 0, ID_SLIDER_VOLUME);
    ...
    /* Media connection */
    WM_CreateWindowAsChild(558, 252, 1, 1, pMsg->hWin, WM_CF_SHOW, _cbMediaConnection , 0);
...
static void Startup(WM_HWIN hWin, uint16_t xpos, uint16_t ypos)
{
  ...
  {    
    ...
    hMainList = WM_CreateWindowAsChild(0, 0, 800, 480, hWinAudioPlayer, WM_CF_HIDE, _cbMainListDialog, 0);

    hMainVisEq = WM_CreateWindowAsChild(240, 70, VIZ_WIDTH, VIZ_HEIGHT, hWinAudioPlayer, (WM_CF_HIDE|WM_CF_FGND|WM_CF_STAYONTOP), _cbMainVisEqDialog, 0);
  }
}
