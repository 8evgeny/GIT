...
{
  { WINDOW_CreateIndirect, "Window", VIDEO_FRAME_ID, 0, 0, 800, 480, (WM_CF_SHOW | WM_CF_HASTRANS), 0x0, 0 },
};
...
static const GUI_WIDGET_CREATE_INFO _aMessageCreate[] = {
  { WINDOW_CreateIndirect   , "MessageBox"    , ID_WINDOW_0 ,   0 ,   0 , 500 ,  90 , (WM_CF_SHOW | WM_CF_HASTRANS | WM_CF_FGND), 0x0 , 0 },
  { TEXT_CreateIndirect     , "Message"       , ID_TEXT_0   ,   0 ,   0 , 500 ,  40 , (GUI_TA_HCENTER | GUI_TA_VCENTER)  , 0x0 , 0 },
};
...
static void _cbplaybackwin(WM_MESSAGE * pMsg) 
{
  ...
  switch (pMsg->MsgId) 
  {
    case WM_CREATE:   
      ...
      hItem = BUTTON_CreateEx(235, 230, 71, 63, pMsg->hWin, WM_CF_SHOW, 0, ID_BUTTON_PREV);
      WM_SetCallback(hItem, _cbButton_back);
      
      hItem = BUTTON_CreateEx(330, 170, 140, 140, pMsg->hWin, WM_CF_SHOW, 0, ID_BUTTON_PLAY);
      WM_SetCallback(hItem, _cbButton_play);
          
      hItem = BUTTON_CreateEx(545, 230, 71, 63, pMsg->hWin, WM_CF_SHOW, 0, ID_BUTTON_NEXT);
      WM_SetCallback(hItem, _cbButton_fore);
      
      hItem = BUTTON_CreateEx(5, 260, 77, 77, pMsg->hWin, WM_CF_SHOW, 0, ID_BUTTON_HW_JPEG);
      WM_SetCallback(hItem, _cbButton_hw_jpeg); 
     
      hItem = BUTTON_CreateEx(5, 345, 77, 77, pMsg->hWin, WM_CF_SHOW, 0, ID_BUTTON_CHROMART);
      WM_SetCallback(hItem, _cbButton_chromart);    
      
      hItem = BUTTON_CreateEx(740, 5, 55, 70, pMsg->hWin, WM_CF_SHOW, 0, ID_BUTTON_MENU);
      WM_SetCallback(hItem, _cbButton_menu);      
      
      /* Elapsed Time Initialization */
      hItem = TEXT_CreateEx(40, 435, 40, 20, pMsg->hWin, WM_CF_SHOW, 0, ID_ELAPSED_TIME, "00:00");
      TEXT_SetFont(hItem, GUI_FONT_16B_1);
      TEXT_SetTextColor(hItem, 0xFF3C3D48);
      
      /* Total Time Initialization */
      hItem = TEXT_CreateEx(730, 435, 40, 20, pMsg->hWin, WM_CF_SHOW, 0, ID_TOTAL_TIME, "00:00");
      TEXT_SetFont(hItem, GUI_FONT_16B_1);
      TEXT_SetTextColor(hItem, 0xFF3C3D48);
      
      hItem = TEXT_CreateEx(17, 26, 80, 15, pMsg->hWin, WM_CF_SHOW, TEXT_CF_LEFT, ID_FPS, "Rate: 25 fps");
      TEXT_SetFont(hItem, GUI_FONT_13B_1);
      TEXT_SetTextColor(hItem, GUI_WHITE);

      hItem = TEXT_CreateEx(110, 26, 85, 15, pMsg->hWin, WM_CF_SHOW, TEXT_CF_LEFT, ID_FEATURES_CPU, "CPU load: 0%");
      TEXT_SetFont(hItem, GUI_FONT_13B_1);
      TEXT_SetTextColor(hItem, GUI_WHITE); 
      
      hItem = SLIDER_CreateEx(80, 437, 640, 12, pMsg->hWin, WM_CF_SHOW, 0, ID_SLIDER_DURATION);
      ...
      /* Volume slider */
      hItem = SLIDER_CreateEx(740, 130, 12, 280, pMsg->hWin, WM_CF_SHOW, SLIDER_CF_VERTICAL, ID_SLIDER_VIDEO_VOLUME);
      SLIDER_SetBkColor(hItem, GUI_TRANSPARENT);
      SLIDER_SetFocusColor(hItem, 0xFF8080FF);
    ...
    case WM_TIMER:
      {
        ...
        if (Id == ID_TIMER_TOUCH)
        {
          if(TouchWindow == 0)
          {
            TouchWindow = WM_CreateWindowAsChild(70, 0, 730, 480, WM_GetDesktopWindowEx(1), WM_CF_SHOW | WM_CF_HASTRANS | WM_CF_STAYONTOP, _cbTouch , 0);
          }
    ...
    case WM_PAINT:
      ...
      GUI_FillRect(10, 20, 95, 45);
      GUI_FillRect(100, 20, 200, 45);
      break;    
...
static void _cbDialog_info(WM_MESSAGE * pMsg) {
  ...
  {
    case WM_INIT_DIALOG:
      MediaConnectionWin = WM_CreateWindowAsChild(799, 479, 1, 1, pMsg->hWin, WM_CF_SHOW, _cbMediaConnection , 0);
      hItem = BUTTON_CreateEx(740, 5, 55, 70, pMsg->hWin, WM_CF_SHOW, 0, ID_BUTTON_MENU);
      WM_SetCallback(hItem, _cbButton_menu); 
      
      hItem = TEXT_CreateEx(230, 195, 520, 40, pMsg->hWin, WM_CF_SHOW, 0, ID_TEXT_FILENAME, "Please add video files in the selected media storage");
  ...
  switch (pMsg->MsgId) 
  {
    case WM_INIT_DIALOG:
      ...
      {  
        GUI_SelectLayer(1); 
        VisLayer = 1;
        playbackwin = WM_CreateWindowAsChild(0, 0, 800, 480, WM_GetDesktopWindowEx(1), (WM_CF_SHOW | WM_CF_HASTRANS), _cbplaybackwin , 0);
        GUI_SelectLayer(0);
        VideoPlayer_State = VIDEO_PLAY;  
