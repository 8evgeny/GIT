...
uint32_t SRC_Buffer_LED1_Toggle[2]  =
                                      {  0xFF & (~LED1_PIN), /*Value for LED 1 ON  */
                                         0xFF                /*Value for LED 1 OFF */
                                      };
...
int main(void)
{
  ...
  */
  ...
  to GPIOK ODR register in order to turn LED1 On/Off each time comes a request from the DMAMUX request generator */
  HAL_DMA_Start_IT(&DMA_Handle, (uint32_t)SRC_Buffer_LED1_Toggle, (uint32_t)&GPIOK->ODR, 2);
