...
static void BSP_LCD_Enable(void)
{
  static uint8_t LCD_Enabled = 0;
  ...
  if(!LCD_Enabled)
  {
    ...
    gpio_init_structure.Pin       = GPIO_PIN_0;     /* LCD_BL_CTRL pin has to be manually controlled */
    gpio_init_structure.Mode      = GPIO_MODE_OUTPUT_PP;
    ...
    /* Asssert backlight LCD_BL_CTRL pin */
...
{
  /* DMA2D Init needed for DeInit later */
...
static void _ClearCacheHook(U32 LayerMask) {
 ...
 for (i = 0; i < GUI_NUM_LAYERS; i++) {
  if (LayerMask & (1 << i))
  {
    if (_aPendingBuffer[i] >= 0)
    {
      SCB_CleanDCache_by_Addr ((uint32_t *)LCD_GetBufferAddress(i, _aPendingBuffer[i]), (_aySize[i] * _aySize[i] * _aBytesPerPixels[i]));
    }
  }
 }
}
