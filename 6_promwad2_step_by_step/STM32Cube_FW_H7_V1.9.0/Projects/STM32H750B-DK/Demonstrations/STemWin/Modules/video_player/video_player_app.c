...
static uint32_t DecodeBufferAddress , DisplayBufferAddress;
...
void HW_JPEG_Init(void)
{
  ...
  /* Initialize Decode and Display Buffers */
    ...
    if( event.status == osEventMessage )
    {
      switch(event.value.v)
      {
        case DECODE_CONVERT_OUTPUT_BUFF:
          if(hwjpeg_context.enable_dma2d == 1)
          {
            DMA2D_CopyBuffer_YCbCr_To_RGB(0, (const void *)DisplayBufferAddress, NULL, 480, 272, DMA2D_CSS_420);
          ...
          else
          {
            ...
            MCU_BlockIndex += pConvert_Function( (uint8_t *)DecodeBufferAddress, (uint8_t *)DisplayBufferAddress, MCU_BlockIndex, MAX_CHUNK_SIZE, &ConvertedDataCount);
            if((MCU_BlockIndex == MCU_TotalNb) && (MCU_TotalNb != 0))
            {
              _LCD_DrawBitmap16bpp(0, 0, 0, (uint16_t *)DisplayBufferAddress, 480, 272, 960);
              DisplayBufferAddress = (DisplayBufferAddress == (uint32_t )&VIDEO_RGBBuffer[1] ? (uint32_t )&VIDEO_RGBBuffer[0] : (uint32_t )&VIDEO_RGBBuffer[1]);
            }
            else
            {
...
U32 HW_JPEG_Draw (const void * pFileData, U32 DataSize, U32 x0, U32 y0)
{
  ...
  hwjpeg_context.enable_dma2d = toggle_chromART;
...
static void JPEG_DataReadyCallback (JPEG_HandleTypeDef *hjpeg, uint8_t *pDataOut, uint32_t OutDataLength)
{
  ...
  else
  {
    ...
    HAL_JPEG_ConfigOutputBuffer(hjpeg, (uint8_t*)DecodeBufferAddress, MAX_CHUNK_SIZE);
...
static void JPEG_InfoReadyCallback(JPEG_HandleTypeDef *hjpeg, JPEG_ConfTypeDef *pInfo)
{
  if(hwjpeg_context.enable_dma2d == 0)
  {
