...
int main(void)
{
  ...
  while (1)
  {
    ...
    UTIL_LCD_DisplayStringAt(0, 240, (uint8_t *)"Display Foreground Image ", CENTER_MODE); 
    ...
    UTIL_LCD_DisplayStringAt(0, 240, (uint8_t *)"Display Background Image ", CENTER_MODE);
    ...
    UTIL_LCD_DisplayStringAt(0, 240, (uint8_t *)"Display Blended Image    ", CENTER_MODE);
  ...
  if(HAL_DMA2D_Init(&Dma2dHandle) == HAL_OK)
  {
    if(HAL_DMA2D_ConfigLayer(&Dma2dHandle, 1) == HAL_OK)
    {
      if (HAL_DMA2D_Start(&Dma2dHandle, (uint32_t)pSrc, (uint32_t)pDst, xSize, ySize) == HAL_OK)
      {
        ...
        hal_status = HAL_DMA2D_PollForTransfer(&Dma2dHandle, 25);
