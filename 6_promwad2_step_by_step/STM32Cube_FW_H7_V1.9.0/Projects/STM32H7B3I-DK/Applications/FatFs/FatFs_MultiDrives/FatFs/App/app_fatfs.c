...
FATFS SDRAMDISKFatFS;    /* File system object for SRAMDISK logical drive */
FIL SDRAMDISKFile;       /* File object for SRAMDISK */
char SDRAMDISKPath[4];   /* SRAMDISK logical drive path */
/* USER CODE BEGIN PV */
...
static int32_t FS_SDRAMFileOperations(void);
/* USER CODE END PFP */
...
int32_t MX_FATFS_Init(void) 
{
  ...
  SDRAMDISKFatFS.fs_type = FS_FAT16;   
  if (FATFS_LinkDriver(&SD_Driver, SDPath) != 0)
  /* USER CODE BEGIN FATFS_Init */
  {
    return APP_ERROR;
  }
  if (FATFS_LinkDriver(&SDRAMDISK_Driver, SDRAMDISKPath) != 0) 
  /* USER CODE BEGIN FATFS_Init */
  {
    return APP_ERROR;
  }
...
int32_t MX_FATFS_Process(void)
{
  ...
  int32_t process_res = APP_OK, SD_res = APP_OK, SDRAM_res = APP_OK;
  ...
  case APPLICATION_INIT:
    if(BSP_SD_IsDetected(0) == SD_PRESENT)
    {
      ...
      if (f_mkfs(SDRAMDISKPath, FM_ANY, 0, workBuffer, sizeof(workBuffer))!= FR_OK)
      {
        process_res = APP_ERROR;
      }
      else
  ...
  case APPLICATION_RUNNING:
    ...
    SDRAM_res = FS_SDRAMFileOperations();
    if ((SD_res != APP_OK) || (SDRAM_res != APP_OK))
    {
      process_res = APP_ERROR;
    }
...
static int32_t FS_SDRAMFileOperations(void)
{
  ...
  /* Register the file system object to the FatFs module */
  if(f_mount(&SDRAMDISKFatFS, (TCHAR const*)SDRAMDISKPath, 0) == FR_OK)
  {
    /* Create and Open a new text file object with write access */
    if(f_open(&SDRAMDISKFile, "1:STM32.TXT", FA_CREATE_ALWAYS | FA_WRITE) == FR_OK)
    {
      /* Write data to the text file */
      res = f_write(&SDRAMDISKFile, wtext, sizeof(wtext), (void *)&byteswritten);
      ...
      if((byteswritten > 0) && (res == FR_OK))
      {
        ...
        /* Open the text file object with read access */
        if(f_open(&SDRAMDISKFile, "1:STM32.TXT", FA_READ) == FR_OK)
        {
          /* Read data from the text file */
          res = f_read(&SDRAMDISKFile, rtext, sizeof(rtext), (void *)&bytesread);
