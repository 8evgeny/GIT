...
static void HDP_Appli_Config(void)
{
  ...
  ButtonState = 0;
  /* Display LCD messages */
  UTIL_LCD_DisplayStringAt(0, 90 , (uint8_t *)"The HDP_Appli is loaded at 0x08100000", CENTER_MODE);
  UTIL_LCD_DisplayStringAt(0, 105 , (uint8_t *)"The blue LED is ON", CENTER_MODE);
  BSP_LED_Toggle(LED2);    /* Switch on the blue led */
  HAL_Delay(3000); 
  ...
  StepCounter = 0;
   ...
   /* Code executed when the security bit is set and the secure area and RDP level are not set  */
    if((ButtonState != 0) && (StepCounter == 0) && (security_bit == 1) && (security_area_set == 0) && (rdp_level == 0))
    {
      ...
      HAL_Delay(2000);
      UTIL_LCD_DisplayStringAt(0, y_size / 2 + 75 ,(uint8_t *)"Between 0x08000000 and 0x080FFFF0", CENTER_MODE);
      HAL_Delay(2000);
