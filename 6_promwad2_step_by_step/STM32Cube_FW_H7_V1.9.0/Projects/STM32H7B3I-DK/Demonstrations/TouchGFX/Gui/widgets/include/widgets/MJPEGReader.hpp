...
*
*/
class MJPEGReader : public PixelDataWidget
{
public:
    MJPEGReader();
    ...
    bool closeFile();
    bool setMovie(const uint8_t* movie, const uint32_t length);
    ...
    bool isEOF()
    {
        return (headerRead && (frameNumber >= numberOfFrames));
    }
    bool isHWDecodingEnabled()
    {
        return useHWDecoding;
    }
    ...
    void setMovieEndedAction(GenericCallback<>& action)
    {
        movieEndedAction = &action;
    }
private:
    /**
    ...
    /**
     *
     * @brief Decode one frame to outputBuf
     * @arg mode 0 decode in software, 1 attempt decoding in hardware then fallback to software
     */
    bool decodeFrame(uint32_t offset, uint8_t* const outputBuf = 0, int mode = 1);
