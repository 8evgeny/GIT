...
for test (located in main.h):
 - "WAVEFORM_VOLTAGE_GENERATION_FOR_TEST" defined: For this example purpose, generates a
   waveform voltage by DAC peripheral on a spare DAC channel: DAC_CHANNEL_1 on pin PA.04 (Arduino connector CN20 pin A0).
   If ADC channel and DAC channel are selected on the same GPIO:
...
ADC conversion results:
 - ADC conversions results are transferred automatically by DMA, into variable
   ...
   - ADC master results in the 16 LSB [15:0] (with ADC resolution 16 bits)
   - ADC slave results in the 16 MSB [31:16] (with ADC resolution 16 bits)
...
Board settings:
  ADC1 and ADC2 are configured to convert the same channel: ADC_CHANNEL_4 on pin PC.04 (Arduino connector CN20 pin A1).
...
STM32H7B3I-DK board LEDs are used to monitor the program execution status:
...
@par Directory contents 
  ...
  - ADC/ADC_DualModeInterleaved/Inc/stm32h7b3i_discovery_conf.h    HAL configuration file  
