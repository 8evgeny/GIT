...
First , an image of size 480x270 in RGB565 format featuring ST logo is loaded into the OSPI using STM32CubeProgrammer.
...
The APB clock dividers for CD Domain APB3 peripherals, CD Domain APB1 and APB2 peripherals and SRD Domain APB4 peripherals to run at 140MHz.
 ...
 of size 4800x270 in RGB565 format featuring ST slogan surrounded by a white frame (color key RGB565==0xFFFFFF) is displayed on LCD.

 Color keying is enabled/disabled on the image when the tamper button is pressed.
 The effect is that at each button press the white color is made transparent or not thanks
 ...
 For the pixels that match the color key RGB value, all channels (RGB) of those pixels
...
STM32H7B3I DISCO board's LED can be used to monitor the transfer status:
...
Display, LTDC, Display Controller, TFT, LCD, Graphics, RGB565, BMP, color keying,

@par Directory contents

    - LTDC/LTDC_ColorKeying_FromOSPI/Inc/stm32h7b3i_discovery_conf.h STM32H7B3I_DISCO board configuration file
    ...
    - LTDC/LTDC_ColorKeying_FromOSPI/Inc/RGB565_480x270.bin          Image of size 4800x2720 and format RGB565 to be loaded to OSPI and to be displayed on LCD
...
- Step1: 
  ...
  - Select External Flash loader "MX25LM51245G_STM32H7B3I-DISCO".
  - choose the binary image "RGB565_480x270.bin"
