...
int main(void)
{
  ...
  while (1)
  {
    ...
    /*##-4- Make LED2 blink at 1Hz while Alarm still not set off #############*/
    if (AlarmStatus == RESET)
    {
      /* Change the state of LED2 (switch on if LED2 was off and vice-versa) */
      BSP_LED_Toggle(LED2);
      /* Wait during 500ms (to make LED2 blink at 1Hz) */
      HAL_Delay(500);
    }
  }
}
