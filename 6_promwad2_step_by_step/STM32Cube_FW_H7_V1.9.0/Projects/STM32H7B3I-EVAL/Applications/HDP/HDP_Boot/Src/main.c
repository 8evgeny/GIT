...
int main(void)
{
   /*  -------------------------------------------------------
     ...
     The HDP_Boot application should be loaded at address 0x08000000
     --------------------------------------------------------*/
  ...
  */
  ...
  /* Initialize the green and red leds of the evaluation board */
  BSP_LED_Init(LED1);
  BSP_LED_Init(LED2);
...
/**
* @brief  Display_LCD screen background function : Display STLogo, messages
* @retval None
*/
...
static void HDP_Boot_Config(void)
{
  ...
  /* Display LCD messages */
  UTIL_LCD_DisplayStringAt(0, y_size / 2 - 60, (uint8_t *)"Press the TAMPER button to start :", CENTER_MODE);
  ...
  StepCounter = 0;
   ...
   /* Code executed when the security bit, RDP level and the secure area are not set */
    if((ButtonState != 0) && (StepCounter == 0) && (security_bit == 0) && (security_area_set == 0) && (rdp_level == 0))
    {
      ...
      HAL_Delay(2000);
      BSP_LED_Toggle(LED1); /* green led*/
      UTIL_LCD_DisplayStringAt(0, y_size / 2 , (uint8_t *)"The green LED is ON", CENTER_MODE);
      HAL_Delay(2000);
      UTIL_LCD_DisplayStringAt(0, y_size / 2 + 30 , (uint8_t *)"Press the TAMPER button to jump into the HDP_Appli", CENTER_MODE);
      HAL_Delay(2000);
      UTIL_LCD_SetBackColor(UTIL_LCD_COLOR_WHITE);
   ...
   /* Code executed when the security bit and the secure area are not set but the RDP is set to level 1 */
    if((ButtonState != 0) && (StepCounter == 0) && (security_bit == 0) && (security_area_set == 0) && (rdp_level == 1))
    {
      ...
      UTIL_LCD_SetBackColor(UTIL_LCD_COLOR_BLUE);
      UTIL_LCD_SetTextColor(UTIL_LCD_COLOR_WHITE);
      UTIL_LCD_DisplayStringAt(0, y_size / 2 + 60 , (uint8_t *)"Press the TAMPER button to jump into the HDP_Appli", CENTER_MODE);
      StepCounter = StepCounter +1 ;
      ButtonState = 0;
    }
   ...
   /* Code executed when the security bit is set and the secure area and RDP level are not set  */
    if((ButtonState != 0) && (StepCounter == 0) && (security_bit == 1)&& (security_area_set == 0) && (rdp_level == 0))
    {
      ...
      UTIL_LCD_SetBackColor(UTIL_LCD_COLOR_WHITE);
      UTIL_LCD_SetTextColor(UTIL_LCD_COLOR_RED);
      UTIL_LCD_DisplayStringAt(0, y_size / 2 - 30 , (uint8_t *)"The secure area not yet set !!", CENTER_MODE);
      HAL_Delay(2000);
      UTIL_LCD_SetBackColor(UTIL_LCD_COLOR_BLUE);
      UTIL_LCD_SetTextColor(UTIL_LCD_COLOR_WHITE);
      ...
      HAL_Delay(2000);
      BSP_LED_Toggle(LED1); /* green led*/
      UTIL_LCD_DisplayStringAt(0, y_size / 2 + 15 , (uint8_t *)"The green LED is ON", CENTER_MODE);
      HAL_Delay(2000);
      UTIL_LCD_DisplayStringAt(0, y_size / 2 + 45 , (uint8_t *)"Press the TAMPER button to jump into the HDP_Appli", CENTER_MODE);
      HAL_Delay(2000);
      UTIL_LCD_SetTextColor(UTIL_LCD_COLOR_YELLOW);
   ...
   /* Code executed when both the security bit, the secure area are set and RDP level is not set*/
    if((ButtonState != 0) && (StepCounter == 0) && (security_bit == 1) && (security_area_set != 0) && (rdp_level == 0))
    {
      ...
      Display_background ();
      BSP_LED_Toggle(LED1); /* green led*/
      UTIL_LCD_DisplayStringAt(0, y_size / 2 - 30 , (uint8_t *)"The green LED will be ON", CENTER_MODE);
      HAL_Delay(2000);
      UTIL_LCD_DisplayStringAt(0, y_size / 2  , (uint8_t *)"Press the TAMPER button to jump into the HDP_Appli", CENTER_MODE);
      HAL_Delay(2000);
      UTIL_LCD_SetTextColor(UTIL_LCD_COLOR_YELLOW);
   ...
   /* Code executed when the security bit, the secure area and the RDP level 1 are set */
    if((ButtonState != 0) && (StepCounter == 0) && (security_bit == 1) && (security_area_set != 0) && (rdp_level == 1))
    {
      ...
      UTIL_LCD_SetBackColor(UTIL_LCD_COLOR_BLUE);
      UTIL_LCD_SetTextColor(UTIL_LCD_COLOR_WHITE);
      UTIL_LCD_DisplayStringAt(0, y_size / 2 + 65 , (uint8_t *)"Press the TAMPER button to jump into the HDP_Appli", CENTER_MODE);
      StepCounter = StepCounter +1 ;
      ButtonState = 0;  
    }
