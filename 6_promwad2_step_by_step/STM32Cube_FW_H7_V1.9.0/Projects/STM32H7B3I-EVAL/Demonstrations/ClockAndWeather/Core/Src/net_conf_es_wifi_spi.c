...
static uint32_t SPI_WIFI_MspInit(SPI_HandleTypeDef *hspi);

/* Private define ------------------------------------------------------------*/

#define DATA_READY_IRQ  MFX_IRQOUT_EXTI_IRQn

#define WIFI_RESET_MODULE()                do{\
                                               BSP_IO_WritePin(0, IO_PIN_15, IO_PIN_RESET);\
                                               HAL_Delay(10);\
                                               BSP_IO_WritePin(0, IO_PIN_15, IO_PIN_SET);\
                                               HAL_Delay(500);\
                                             }while(0);
...
{ 
  ...
  uint8_t tmp;
  while(hbus_i2c2.State != HAL_I2C_STATE_READY)
  {
    conflict++;
    osDelay(1);
  }
  ...
  if(BSP_IO_ReadPin(0, IO_PIN_2) == IO_PIN_2)
    tmp = 1;
  else
    tmp = 0;
  __enable_irq();
...
static uint32_t SPI_WIFI_MspInit(SPI_HandleTypeDef *hspi)
{
    ...
    io_init_structure.Pin  = IO_PIN_10;
  ...
  {
    if( BSP_IO_WritePin( 0, IO_PIN_10, IO_PIN_RESET )!= BSP_ERROR_NONE )  
      ret = BSP_ERROR_NO_INIT;
  }

    /* configure Data ready pin */
    io_init_structure.Pin  = IO_PIN_2;
    io_init_structure.Pull = IO_NOPULL;
...
static int8_t SPI_WIFI_Init(uint16_t mode)
{
    ...
    if (mode == ES_WIFI_INIT)
    {
        ...
        hspi.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_16; /* 80/8= 10MHz (Inventek WIFI module supports up to 20MHz)*/
        ...
        HAL_NVIC_SetPriority((IRQn_Type)SPI4_IRQn, SPI_INTERFACE_PRIO, 0);
        HAL_NVIC_EnableIRQ((IRQn_Type)SPI4_IRQn);
