...
void k_InitMenu(void)
{
  ...
  hItem = BUTTON_CreateEx(145, 135, 210, 210, hbuttons , WM_CF_SHOW | WM_CF_HASTRANS, 0 , ID_BUTTON_MODULE1);
  WM_SetCallback(hItem, _cbButton_module1);

  hItem = BUTTON_CreateEx(445, 135, 210, 210, hbuttons , WM_CF_SHOW | WM_CF_HASTRANS, 0 , ID_BUTTON_MODULE2);
  WM_SetCallback(hItem, _cbButton_module2);

  hItem = BUTTON_CreateEx(720, 420, 75, 60, hbuttons , WM_CF_SHOW | WM_CF_HASTRANS, 0 , ID_BUTTON_INFO);
  WM_SetCallback(hItem, _cbButton_info);

  hItem = TEXT_CreateEx(480, 440, 225, 20, hbuttons , WM_CF_HIDE, TEXT_CF_LEFT, ID_TEXT_STORAGE, "Please insert media storage");
  TEXT_SetFont(hItem, GUI_FONT_20_1);
  TEXT_SetTextColor(hItem, GUI_WHITE);
}
