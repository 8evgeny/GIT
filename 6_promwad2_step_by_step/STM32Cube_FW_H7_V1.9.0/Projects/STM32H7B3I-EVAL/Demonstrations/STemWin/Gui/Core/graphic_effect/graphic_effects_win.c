...
static const GUI_WIDGET_CREATE_INFO _aDialogInfo[] = {
  { WINDOW_CreateIndirect, "Info Window", ID_WINDOW_0, 0, 0, 800, 480, (WM_CF_SHOW | WM_CF_STAYONTOP), 0x64, 0 },
};

static const GUI_WIDGET_CREATE_INFO _aDialogGraphicEffects[] = {
  { WINDOW_CreateIndirect, "GraphicEffects Window", ID_WINDOW_0, 0, 0, 800, 480, WM_CF_SHOW, 0x64, 0 },
};
...
static void _OnPaint_PreviousButton(BUTTON_Handle hObj)
{
  if (SrcImageIdx == 0)
  {
      GUI_DrawBitmap(&bmPreviousGrayButton, 17, 30);
  }
  else
  {
    if ((WIDGET_GetState(hObj) & BUTTON_STATE_PRESSED))
    {
        GUI_DrawBitmap(&bmPreviousPressedButton, 17, 30);
    }
    else
    {
        GUI_DrawBitmap(&bmPreviousButton, 17, 30);
    }
  }
}
...
static void _cbWinGraphicEffects(WM_MESSAGE * pMsg)
{
  ...
  {
    case WM_INIT_DIALOG:
      IMAGE_CreateEx(0,  0, 800, 480, pMsg->hWin, WM_CF_SHOW, 0, ID_IMAGE_GEINFO);
      
      WM_CreateWindowAsChild(799, 479, 1, 1, pMsg->hWin, WM_CF_SHOW, _cbMediaConnection , 0);
      
      hItem = BUTTON_CreateEx(730, 5, 65, 70, pMsg->hWin, WM_CF_SHOW, 0, ID_BUTTON_MAIN_MENU);
      ...
      hItem = BUTTON_CreateEx(110, 480 - bmEdgeDetection.YSize - 24, bmEdgeDetection.XSize, bmEdgeDetection.YSize + 20, pMsg->hWin, WM_CF_SHOW, 0, ID_BUTTON_EDGE_DETECTION);
      WM_SetCallback(hItem, _cbButtons);

      hItem = BUTTON_CreateEx(110 + 20 + bmEdgeDetection.XSize, 480 - bmMotionBlur.YSize - 24, bmMotionBlur.XSize, bmMotionBlur.YSize + 20, pMsg->hWin, WM_CF_SHOW, 0, ID_BUTTON_MOTION_BLUR);
      WM_SetCallback(hItem, _cbButtons);

      hItem = BUTTON_CreateEx(110 + 20 + 20 + bmEdgeDetection.XSize * 2, 480 - bmRaising.YSize - 24, bmRaising.XSize, bmRaising.YSize + 20, pMsg->hWin, WM_CF_SHOW, 0, ID_BUTTON_RAISING);
      WM_SetCallback(hItem, _cbButtons);

      hItem = BUTTON_CreateEx(110 + 20 + 20 + 20 + bmEdgeDetection.XSize * 3, 480 - bmSharpening.YSize - 24, bmSharpening.XSize, bmSharpening.YSize + 20, pMsg->hWin, WM_CF_SHOW, 0, ID_BUTTON_SHARPENING);
      WM_SetCallback(hItem, _cbButtons);

      hItem = BUTTON_CreateEx(110 + 20 + 20 + 20 + 20 + bmEdgeDetection.XSize * 4, 480 - bmSmoothing.YSize - 24, bmSmoothing.XSize, bmSmoothing.YSize + 20, pMsg->hWin, WM_CF_SHOW, 0, ID_BUTTON_SMOOTHING);
      WM_SetCallback(hItem, _cbButtons);

      hItem = BUTTON_CreateEx(-15, 480 - bmPreviousButton.YSize - 4 - 30, bmPreviousButton.XSize + 30, bmPreviousButton.YSize + 30, pMsg->hWin, WM_CF_SHOW, 0, ID_BUTTON_PREVIOUS);
      WM_SetCallback(hItem, _cbButtons);

      hItem = BUTTON_CreateEx(800 - 2 - bmNextButton.XSize - 30, 480 - bmNextButton.YSize - 4 - 30, bmNextButton.XSize + 30, bmNextButton.YSize + 30, pMsg->hWin, WM_CF_SHOW, 0, ID_BUTTON_NEXT);
      WM_SetCallback(hItem, _cbButtons);

      hItem = TEXT_CreateEx(368, 12, 150, 20, pMsg->hWin, WM_CF_SHOW, TEXT_CF_LEFT, ID_TEXT_FEATURES_CPU, "CPU load: 0%");
    ...
    case WM_PAINT:
      ...
      GUI_DrawBitmap(&bmCpuLoadBackground, 354, 4);
      break;
      ...
      switch (pMsg->Data.v)
      {
        ...
        else
        {
          ...
          switch(id)
          {
            case ID_BUTTON_EDGE_DETECTION:
            {
              ...
              GUI_RECT filterRect = { 0, 430, 800, 480 };
              WM_InvalidateRect(pMsg->hWin, &filterRect);
            }
            break;
            case ID_BUTTON_MOTION_BLUR:
            ...
            case ID_BUTTON_NEXT:
            {
              if (SrcImageIdx < (BMPList.ptr - 1))
              {
                ...
                LCD_DrawBitmap_Effect(storage_id, 0, 800, 480, (uint8_t *)&(GFX_Buffers.buffers[SrcImageIdx + 5].pixels), (uint32_t *)&(GFX_Buffers.buffers[3].pixels));
                LCD_DrawBitmap_Effect(storage_id, 0, 800, 480, (uint8_t *)&(GFX_Buffers.buffers[3].pixels), (uint32_t *)&(GFX_Buffers.buffers[4].pixels));

                GUI_RECT filterRect = { 0, 0, 800, 480 };
                WM_InvalidateRect(pMsg->hWin, &filterRect);
        ...
        case WM_NOTIFICATION_RELEASED:
          ...
          switch(id)
          {
            ...
            case ID_BUTTON_RESET_DST_IMAGE:
            {
              LCD_DrawBitmap_Effect(storage_id, 0, 800, 480, (uint8_t *)&(GFX_Buffers.buffers[3].pixels), (uint8_t *)&(GFX_Buffers.buffers[4].pixels));
              GUI_RECT filterRect = { 0, 0, 800, 480 };
              WM_InvalidateRect(pMsg->hWin, &filterRect);
            }
            break;
...
static void _cbDialog_info(WM_MESSAGE * pMsg) {
  ...
  {
    case WM_INIT_DIALOG:
      hItem = BUTTON_CreateEx(730, 5, 65, 70, pMsg->hWin, WM_CF_HIDE, 0, ID_BUTTON_MAIN_MENU);
      WM_SetCallback(hItem, _cbButtons);
      
      hItem = TEXT_CreateEx(230, 195, 520, 40, pMsg->hWin, WM_CF_SHOW, 0, ID_TEXT_FILENAME, dialog_msg);
...
static void Startup(WM_HWIN hWin, uint16_t xpos, uint16_t ypos)
{
  ...
  if(BMPList.ptr == 0)
  {
    ...
    if(bmp_count == 3)
    {
      ...
      LCD_DrawBitmap_Effect(storage_id, 1, 800, 480, (uint8_t *)&(GFX_Buffers.buffers[0].header), (uint32_t *)&(GFX_Buffers.buffers[5].pixels));
      LCD_DrawBitmap_Effect(storage_id, 1, 800, 480, (uint8_t *)&(GFX_Buffers.buffers[1].header), (uint32_t *)&(GFX_Buffers.buffers[6].pixels));
      LCD_DrawBitmap_Effect(storage_id, 1, 800, 480, (uint8_t *)&(GFX_Buffers.buffers[2].header), (uint32_t *)&(GFX_Buffers.buffers[7].pixels));
    }
  }
  ...
  if(bmp_count == 3)
  {
    ...
    LCD_DrawBitmap_Effect(storage_id, 0, 800, 480, (uint8_t *)&(GFX_Buffers.buffers[6].pixels), (uint32_t *)&(GFX_Buffers.buffers[3].pixels));
