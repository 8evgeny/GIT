/**
  ...
  *          and exit the STOP2 mode using RTC.
  ******************************************************************************
  * @attention
  *
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* The Following Wakeup sequence is highly recommended prior to each STOP mode entry
    ...
    */
  if (HAL_RTCEx_SetWakeUpTimer_IT(&RTCHandle, 0x9C40, RTC_WAKEUPCLOCK_RTCCLK_DIV16) != HAL_OK)
  {
    Error_Handler();
  }
  ...
  if(__HAL_PWR_GET_FLAG(PWR_FLAG_STOP) == RESET)
  {
    ...
    /* The Flash memory must be configured in low-power mode
       (FLPS bit set in PWR_CR1 register) before entering DStop2. */
    ...
    /* Return the Flash memory to the normal mode. */
