...
/* STM32H7xx HAL library initialization:
  ...
  if(HAL_SD_ConfigSpeedBusOperation(&SDHandle,SDMMC_SPEED_MODE_HIGH) != HAL_OK)
  {
    Error_Handler();
  }
  ...
  while(1)
  {
    switch(step)
    {
      ...
      case 2:
      {
        if(TxCplt != 0)
        {
          /* Toggle LED4 Transfer of Buffer OK */
          BSP_LED_Toggle(LED4);
...
static void SystemClock_Config(void)
{
  ...
  */
  ...
  /* Activation of CSI clock is mandatory for I/O Compensation Cell */  
  __HAL_RCC_CSI_ENABLE() ;
    
  /* Activation of SYSCFG clock is mandatory for I/O Compensation Cell */
  __HAL_RCC_SYSCFG_CLK_ENABLE() ;
  
  /* Enabling the I/O Compensation Cell */    
