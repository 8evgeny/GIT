...
#define Nb 4   // number bytes in word (for 32-bit word =4)
...
//Precalculated table (encryption) MixColumns
// te[i]=[sbox[i]*3 sbox[i]*1 sbox[i]*1 sbox[i]*2] , * - Galua Field mul, sbox[i]*3 = (sbox[i]*2) xor sbox[i]
...
//Precalculated table (decryption) InvMixColumns
// td[i]=[sbox[i]*11 sbox[i]*13 sbox[i]*9 sbox[i]*14] , * - Galua Field mul
...
void CryptoDialog::setKey(uint8_t* key, QCryptoType type){
	typeCrypto = type;
	if (typeCrypto == AES128)
		memcpy(CryptoKey, key, 16); else
	if (typeCrypto == AES192)
		memcpy(CryptoKey, key, 24); else
	if (typeCrypto == AES256)
		memcpy(CryptoKey, key, 32);
}

uint8_t CryptoDialog::KeyInit(uint8_t *key, uint8_t keyLength, AES_State *st)
{
  uint8_t i,keysize;
  uint32_t tmp;
  st->Nk = keyLength >> 2;
  ...
  for (i=0;i<st->Nk;i++) st->ek[i] = ((uint32_t *)key)[i];

  keysize = Nb*(st->Nr+1);

  for (i=st->Nk;i<keysize;i++) {
	tmp = st->ek[i-1];
	if ((i%st->Nk)==0) {
	  st->ek[i]  = sbox[(tmp >> 8) & 0xFF];
	  st->ek[i] |= (sbox[(tmp >> 16) & 0xFF] << 8);
	  st->ek[i] |= (sbox[(tmp >> 24) & 0xFF] << 16);
	  st->ek[i] |= (sbox[tmp & 0xFF] << 24);
	  st->ek[i] ^= rcon[i/st->Nk];
	} else if ((st->Nk>6)&&((i%st->Nk)==4)){
	  st->ek[i] = sbox[tmp & 0xFF];
	  st->ek[i] |= (sbox[(tmp >> 8) & 0xFF] << 8);
	  st->ek[i] |= (sbox[(tmp >> 16) & 0xFF] << 16);
	  st->ek[i] |= (sbox[(tmp >> 24) & 0xFF] << 24);
	} else
	  st->ek[i] = tmp;

	st->ek[i] ^= st->ek[i - st->Nk];
  }
  ...
  for (i=0;i<keysize;i++) {
	...
	{
	  st->dk[i] = td[sbox[st->ek[i] & 0xFF]];
	  tmp = td[sbox[(st->ek[i] >> 8) & 0xFF]];
	  st->dk[i] ^= rol(tmp, 8);
	  tmp = td[sbox[(st->ek[i] >> 16) & 0xFF]];
	  st->dk[i] ^= rol(tmp, 16);
	  tmp = td[sbox[(st->ek[i] >> 24) & 0xFF]];
	  st->dk[i] ^= rol(tmp, 24);
	}
  }
...
void CryptoDialog::Code_Block(uint8_t *block, AES_State *st)
{
  uint32_t s0,s1,s2,s3;
  uint32_t t0,t1,t2,t3,tmp;
  uint8_t i,nr;
...
void CryptoDialog::Decode_Block(uint8_t *block, AES_State *st)
{
  uint32_t s0,s1,s2,s3;
  uint32_t t0,t1,t2,t3,tmp;
  uint8_t i, nr;
...
uint32_t CryptoDialog::EncodeBuffer(uint8_t *src_buff, uint32_t length_src_buff, uint8_t *dst_buff){
#ifdef __IS_PHANTOM_ENC__
		memcpy(dst_buff, src_buff, length_src_buff);
		return length_src_buff;
#endif
	...
	if (typeCrypto == AES256)
		_lenkey = 32;
	if ( KeyInit((uint8_t*)&CryptoKey, _lenkey, (AES_State*)&st) == 1 ){
		for (int _i = 0; _i < rc; _i++){
			memcpy(block, (uint8_t*)&src_buff[_i*AES_BLOCK_SIZE], AES_BLOCK_SIZE);
			Code_Block((uint8_t*)&block, (AES_State*)&st);
			memcpy((uint8_t*)&dst_buff[_i*AES_BLOCK_SIZE], block, AES_BLOCK_SIZE);
		}
		if (rd){
			memset(block, 0, AES_BLOCK_SIZE);
			memcpy(block, (uint8_t*)&src_buff[rc*AES_BLOCK_SIZE], rd);
			Code_Block((uint8_t*)&block, (AES_State*)&st);
			memcpy((uint8_t*)&dst_buff[rc*AES_BLOCK_SIZE], block, AES_BLOCK_SIZE);
			rc++;
		}
		return rc*AES_BLOCK_SIZE;
	}
	return 0;
}

uint32_t CryptoDialog::DecodeBuffer(uint8_t *src_buff, uint32_t length_src_buff, uint8_t *dst_buff){
#ifdef __IS_PHANTOM_ENC__
		memcpy(dst_buff, src_buff, length_src_buff);
		return length_src_buff;
#endif
	...
	if (KeyInit((uint8_t*)&CryptoKey, _lenkey, (AES_State*)&st) == 1){
		for (int _i = 0; _i < rc; _i++){
			memcpy(block, (uint8_t*)&src_buff[_i*AES_BLOCK_SIZE], AES_BLOCK_SIZE);
			Decode_Block((uint8_t*)&block, (AES_State*)&st);
			memcpy((uint8_t*)&dst_buff[_i*AES_BLOCK_SIZE], block, AES_BLOCK_SIZE);
		}
		if (rd){
			memset(block, 0, AES_BLOCK_SIZE);
