...
ServiceViewDialog::ServiceViewDialog(QWidget *parent, CModeExchange mode)
    : QDialog(parent, Qt::FramelessWindowHint)
    , ui(new Ui::ServiceViewDialog)
{
    ui->setupUi(this);
...
ServiceViewDialog::~ServiceViewDialog()
{
...
void ServiceViewDialog::setTitle(QString _title){
	ui->l_title->setText(_title);
}

void ServiceViewDialog::setMode(CModeExchange _mode){
	ModeExchange = _mode;
}

void ServiceViewDialog::setRS232Name(QString _portname){
	portName = _portname;
	serialPort->setPortName(portName);
}

void ServiceViewDialog::setSelfIP(QString _selfip){
	selfIP = _selfip;
	QString strIP;
...
//	qDebug() << QObject::tr("ServiceViewDialog::RxUDPPort=%1").arg(RxUDPPort);
	ip_dec = qFromBigEndian<quint32>(ip_dec);
	strIP = Data2Hex((unsigned char*)&ip_dec, 4);
	strPrepend = QObject::tr("CONFIG : %1 : ").arg(strIP);
}

void ServiceViewDialog::setAesKey(uint8_t* key, uint8_t length_key){
...
void ServiceViewDialog::b_start_clicked()
{
	if (isStart){
		fnServiceOff();
...
void ServiceViewDialog::testTimer_timeout(){
...
void ServiceViewDialog::b_savelog_clicked()
{
    auto file = QFileDialog::getSaveFileName(this, QObject::tr("Save a service log as...")
                                    , QString::fromLocal8Bit(""), QString::fromLocal8Bit("srv")); 

    if (file.isEmpty())
        return;

    if (!file.endsWith(IPS_STR(".srv"))) {
        file = IPS_STR("%1.srv").arg(file);
    }
	model->save(file);
}

void ServiceViewDialog::b_loadlog_clicked(){
    auto const& file = QFileDialog::getOpenFileName(this, QObject::tr("Load service log...")
                                    , QString::fromLocal8Bit(""), QString::fromLocal8Bit("*.srv")); 
...
void ServiceViewDialog::setConfigureStr(QString _data){
	int cidName;
	QString _str;
...
void ServiceViewDialog::cb_liststations_currentIndexChanged(int index){
	Q_UNUSED(index);
	int cidName;
	QString ip;
	QString str;
...
QString ServiceViewDialog::getIPFromCid(QJsonArray* StationList, QString _cid){
...
void ServiceViewDialog::appendMsg(QString _dt, QString _cid, QString _ip, QString _msg){
...
void ServiceViewDialog::serialPort_dataRead()
{
...
void ServiceViewDialog::pTimeTimeouts_timeout()
{
	isTimeoutAnswer = true;
}

void ServiceViewDialog::readPendingDatagrams()
{
	char _tmpbuff[65536];
	qint32 _byte_read = 0; 
...
void ServiceViewDialog::processTheDatagram(QByteArray dt){
	char enc_buff[1500];
	char dec_buff[1500];
	QString _decStr;
...
QString ServiceViewDialog::Data2Hex(unsigned char* _code, unsigned int _length_byte)
{
...
void ServiceViewDialog::fnServiceOn(){
	QString _str;
	char tsrc_buff[1400];
	char tdst_buff[1400];
	...
	isTimeoutAnswer = false;
	isRcvStr = false;
	_str = QString::fromLocal8Bit(CMD_SERVICE_ON);
	if (ModeExchange == RS232Mode){
		serialPort->clear();
...
void ServiceViewDialog::fnServiceOff(){
	QString _str;
	char tsrc_buff[1400];
	char tdst_buff[1400];
	uint8_t _size_enc_buff;
	...
	isTimeoutAnswer = false;
	isRcvStr = false;
	_str = QString::fromLocal8Bit(CMD_SERVICE_OFF);
	if (ModeExchange == RS232Mode){
		serialPort->clear();
		serialPort->write(_str.toLocal8Bit().data(), _str.length());
	} else
