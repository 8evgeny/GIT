...
private:
    static inline auto path() noexcept {
...
public:
    ...
    constexpr explicit Language(language_t l) noexcept
        : m_language(l){
    }
    constexpr explicit Language(uint l) noexcept
    ...
    constexpr language_t lang() const noexcept {
        return m_language;
    }
    constexpr auto toUint() const noexcept {
        return static_cast<uint>(m_language);
    }
    constexpr void setLanguage(language_t l) noexcept {
        m_language = l;
    }
    ...
    static inline QStringList defaultLanguagesNames() noexcept {
        ...
        for(auto const& l: defaultLanguages())
            result << navName(l);

        return result;
    }
    static inline void fill(QComboBox* box) noexcept {
        for(auto const& c: defaultLanguages())
            addItem(box, c);
    }
...
inline uint russianId() noexcept {
    return options::Language::russian().toUint();
}
