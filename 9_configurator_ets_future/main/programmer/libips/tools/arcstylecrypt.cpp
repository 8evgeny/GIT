...
arcCrypt::arcCrypt(const std::vector<uint8_t> &key) :
    cipher (key)
{
}
...
std::vector<uint8_t> const& arcCrypt::toCipher() noexcept
{
    return cipher;
}

void arcCrypt::crypt(void *data, size_t size, const std::vector<uint8_t> &key)
{
  for(uint32_t i = 0;i < size;i++)
    {
      (reinterpret_cast<int*>(data))[i] ^= (key[i % key.size ()]);
    }
}
