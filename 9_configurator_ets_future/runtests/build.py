...
def build(compiler):
    if(compiler == "msvc"):
        ...
        x = os.system("cd bin\ && windeployqt.exe -serialport -multimedia  ips-configurator.exe")
        if(x != 0):
            sys.exit(1)
    elif(compiler == "gcc"):
        rmTree()
        ...
        os.system(f"cd build && cmake --build . --config Release")
        ...
        x = os.system("cd build && cmake --build . --target ips-configurator --config Release")
        if(x != 0):
            sys.exit(1)
    else:
        print(f"Compiler {compiler} is not supported!!!\nOnly msvc supported!!!")
...
if __name__ == '__main__':
    ...
    parser.add_argument('-b', '--build', nargs='?', const='msvc', metavar="compiler (msvc, gcc, clang)", help=f"Build the project")
