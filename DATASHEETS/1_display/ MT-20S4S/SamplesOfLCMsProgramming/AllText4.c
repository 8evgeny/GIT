//Пример программы для буквенно-цифровых индикаторов MT10S1, MT16S1, MT20S1, MT24S1, MT16S2, MT20S2, MT24S2, MT20S4 с подключением по 4-х битному режиму.
...
//При необходимости настроить здесь шину данных на вывод
	LCD.RW=0; LCD.A0=0; LCD.D=0x03;	//Установка типа интерфейса (8 бит) (0x03 - старший полубайт команды 0x30) - в начале ВСЕГДА переключаем индикатор в 8-ми битный режим работы
	...
	LCD.E=1; Delay(>230ns);
	LCD.E=0; Delay(>40us);	//Пауза между командами
	LCD.D=0x02;		//Установка типа интерфейса (4 бит) (0x02 - старший полубайт команды 0x20) (при этом биты RW и A0 измениться не должны!)
	LCD.E=1; Delay(>230ns);	//Время предустановки данных попало сюда (tDSW)
...
//При необходимости настроить здесь шину данных на ввод
	...
	while(LCD.D.3==1);	//Ждать сброса флага занятости
	LCD.E=0; Delay(>270ns);	//Минимально допустимый интервал между сигналами E=1
	LCD.E=1; Delay(>230ns);	//Получим младшую половину байта
	LCD.E=0; Delay(>270ns);	//Минимально допустимый интервал между сигналами E=1
//При необходимости настроить здесь шину данных на вывод
