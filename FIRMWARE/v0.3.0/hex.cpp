...
Hex::FileParseError Hex::loadFromFile(QString fileName)
{
    ...
    if(fileName.endsWith(".bin", Qt::CaseInsensitive)) {
        d = file.readAll();
        file.close();
        base =  FORBIDDEN_ADDRESS;
        start = FORBIDDEN_ADDRESS;
        stringNum = d.size();
        return error = EOF_DETECTED;
    }
    qCritical()<<"UNSUPPORTED_FILE_FORMAT"<<fileName;
	...
	*baseAdr = 0; *startAdr = 0;
	if( !src->isOpen() || !(src->openMode() & QIODevice::ReadOnly) ) {
		...
		if( !src->open(QIODevice::ReadOnly) ) {
			...
			if(file) qCritical()<<file->fileName()<<file->errorString();
	...
	if(elem != sizeof(Elf32_Ehdr)) {
		qCritical("Error: read ELF header, error = %s.", src->errorString().toLocal8Bit().constData());
	...
	if(hdr.e_ident[EI_MAG0] != ELFMAG0 ||
	   ...
	   hdr.e_ident[EI_MAG3] != ELFMAG3 ) {
		qCritical("Error: magic number not found.");
		return UNSUPPORTED_FILE_FORMAT;
	}
	if(hdr.e_ident[EI_CLASS] != ELFCLASS32) {
		qCritical("Error: not supported class, %d.", hdr.e_ident[EI_CLASS]);
		return UNSUPPORTED_FILE_FORMAT;
	}
	if(hdr.e_ident[EI_DATA] != ELFDATA2LSB) {
		qCritical("Error: not supported dataorder, %d.", hdr.e_ident[EI_DATA]);
		return UNSUPPORTED_FILE_FORMAT;
	}
	if(hdr.e_ident[EI_VERSION] != EV_CURRENT) {
		qCritical("Error: not supported VERSION, %d.", hdr.e_ident[EI_VERSION]);
		return UNSUPPORTED_FILE_FORMAT;
	}
	
	
	if(hdr.e_phentsize != sizeof(Elf32_Phdr)) {
		qCritical("Error: program header size from ELF header mismatch whith sizeof(elf32_phdr)");
		return UNSUPPORTED_FILE_FORMAT;
	}
	if(hdr.e_shentsize != sizeof(Elf32_Shdr)) {
		qCritical("Error: section header size from ELF header mismatch whith sizeof(elf32_shdr)");
		return UNSUPPORTED_FILE_FORMAT;
	}
	
	if(hdr.e_phnum == 0) {
		qCritical("Error: program headers count is ZERO.");
		return UNSUPPORTED_FILE_FORMAT;
	}
	if(hdr.e_phnum > 1) {
		qDebug("Info: program headers count is %d.", hdr.e_phnum);
	}
	
	Elf32_Phdr phdr[hdr.e_phnum];
	if( !src->seek(hdr.e_phoff)) {
		qCritical("Error: seek fail, error = %s.", src->errorString().toLocal8Bit().constData());
		return EMPTY_LINE_IS_READED;
	}
	elem = src->read((char*)phdr, sizeof(Elf32_Phdr) * hdr.e_phnum);
	if(elem != sizeof(Elf32_Phdr) * hdr.e_phnum) {
		qCritical("Error: read program header(s), error = %s.", src->errorString().toLocal8Bit().constData());
		return EMPTY_LINE_IS_READED;
	}
	...
	for(i=0; i<hdr.e_phnum; ++i) {
		...
		qDebug("Selected programm header number is %u", i);
		break; // i содержит номер заголовка загружаемого и исполняемого.
	}
	
	if(i >= hdr.e_phnum) {
		qCritical("Error: not found loaded and executable program headers!");
		return UNSUPPORTED_FILE_FORMAT;
	}
	if( !src->seek(phdr[i].p_offset) ) {
		qCritical("Error: seek fail at read data, error = %s.", src->errorString().toLocal8Bit().constData());
		return EMPTY_LINE_IS_READED;
	}
	
	dest.buffer().resize(phdr[i].p_filesz);
	elem = src->read(dest.buffer().data(), phdr[i].p_filesz);
	if(elem != phdr[i].p_filesz) {
		qCritical("Error: read data to buffer, ferror = %s.", src->errorString().toLocal8Bit().constData());
		return EMPTY_LINE_IS_READED;
	}
	*startAdr = hdr.e_entry;
	*baseAdr = phdr[i].p_paddr;
	return EOF_DETECTED;
}
    ...
    while( !src->atEnd() && (status == EOF_ISNT_DETECTED) ) {
        ...
        *stringNumber += 1;
        ...
        if(ccs != (unsigned char)raw.at(raw.size()-1)) {
            qCritical()<<"ccs ="<<ccs<<" cs ="<< (unsigned char)raw.at(raw.size()-1);
