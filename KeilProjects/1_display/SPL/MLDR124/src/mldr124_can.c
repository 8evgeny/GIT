...
void CAN_BRGInit(MDR_CAN_TypeDef* CANx, uint32_t CAN_BRG)
{
    ...
    case ( uint32_t )MDR_CAN0:
        CLK_CNTR->PER0_CLK |= 1 << 24;
        CLK_CNTR->CAN0_CLK =  1<<16 | CAN_BRG;
        break;
    case ( uint32_t )MDR_CAN1:
        CLK_CNTR->PER0_CLK |= 1 << 25;
        CLK_CNTR->CAN1_CLK =  1<<16 | CAN_BRG;        
        break;
    }
}
