...
{
  DMA->CONFIG = 0;                                                                      /* Master Enable Off */
...
#else                                               
  DMA->CHNL_USEBURST_SET = 0xFFFFFFFF;                                                  /* Disable burst mode */ 
  DMA->CHNL_REQ_MASK_SET = 0xFFFFFFFF;                                                  /* Disable all channel*/  
  DMA->CHNL_ENABLE_SET = 0xFFFFFFFF;                                                    /* Clear channel enable */
#endif                                                  
...
void DMA_SG_Init( uint8_t DMA_Channel, DMA_Channel_SG_InitTypeDef *DMA_SG_InitStruct)
{
  ...
  DMA->CONFIG = DMA_CFG_MASTER_ENABLE | DMA_SG_InitStruct->DMA_ProtCtrl;
...
void DMA_Init(uint8_t DMA_Channel, DMA_ChannelInitTypeDef* DMA_InitStruct)
{
  ...
  /* DMA configuration register */
  DMA->CONFIG = DMA_CFG_MASTER_ENABLE | DMA_InitStruct->DMA_ProtCtrl;
...
FlagStatus DMA_GetFlagStatus(uint8_t DMA_Channel, uint8_t DMA_Flag)
{
  ...
  switch(DMA_Flag)
  {
    ...
    case DMA_FLAG_CHNL_WAIT:
      return (FlagStatus)(DMA->WAITONREG_STATUS & (1 << DMA_Channel));
