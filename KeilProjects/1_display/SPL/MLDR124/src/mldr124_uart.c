                          ...
                          UART_InitTypeDef* UART_InitStruct )
{
    ...
    case ( uint32_t )MDR_UART0:
        clocksource = ( CLK_CNTR->UART0_CLK >> 28 ) & 0xF;
        clockdiv = 1 + CLK_CNTR->UART0_CLK & 0xFFFF;
        break;
    case ( uint32_t )MDR_UART1:
        clocksource = ( CLK_CNTR->UART1_CLK >> 28 ) & 0xF;
        clockdiv = 1 + CLK_CNTR->UART1_CLK & 0xFFFF;
        break;
    case ( uint32_t )MDR_UART2:
        clocksource = ( CLK_CNTR->UART2_CLK >> 28 ) & 0xF;    
        clockdiv = 1 + CLK_CNTR->UART2_CLK & 0xFFFF;
        break;
    case ( uint32_t )MDR_UART3:
        clocksource = ( CLK_CNTR->UART3_CLK >> 28 ) & 0xF;   
        clockdiv = 1 + CLK_CNTR->UART3_CLK & 0xFFFF;
        break;
    }    
...
void UART_IrDAConfig(MDR_UART_TypeDef* UARTx, uint32_t UART_IrDAMode)
{
    ...
    if (UART_IrDAMode == UART_IrDAMode_LowPower)
    {
        ...
        case ( uint32_t )MDR_UART0:
            clocksource = ( CLK_CNTR->UART0_CLK >> 28 ) & 0xF;
            clockdiv = CLK_CNTR->UART0_CLK & 0xFFFF;
            break;
        case ( uint32_t )MDR_UART1:
            clocksource = ( CLK_CNTR->UART1_CLK >> 28 ) & 0xF;
            clockdiv = CLK_CNTR->UART1_CLK & 0xFFFF;
            break;
        case ( uint32_t )MDR_UART2:
            clocksource = ( CLK_CNTR->UART2_CLK >> 28 ) & 0xF;    
            clockdiv = CLK_CNTR->UART2_CLK & 0xFFFF;
            break;
        case ( uint32_t )MDR_UART3:
            clocksource = ( CLK_CNTR->UART3_CLK >> 28 ) & 0xF;   
            clockdiv = CLK_CNTR->UART3_CLK & 0xFFFF;
            break;
        }    
...
void UART_BRGInit(MDR_UART_TypeDef* UARTx, uint16_t UART_BRG, uint32_t UART_CLK_SOURCE)
{
    ...
    {
    case ( uint32_t )MDR_UART0:
        CLK_CNTR->PER0_CLK |= 1 << 26;
        CLK_CNTR->UART0_CLK = UART_CLK_SOURCE << 28 | 1<<16 | UART_BRG;
        break;
    case ( uint32_t )MDR_UART1:
        CLK_CNTR->PER0_CLK |= 1 << 27;
        CLK_CNTR->UART1_CLK = UART_CLK_SOURCE << 28 | 1<<16 | UART_BRG;        
        break;
    case ( uint32_t )MDR_UART2:
        CLK_CNTR->PER0_CLK |= 1 << 28;
        CLK_CNTR->UART2_CLK = UART_CLK_SOURCE << 28 | 1<<16 | UART_BRG;        
        break;
    case ( uint32_t )MDR_UART3:
        CLK_CNTR->PER0_CLK |= 1 << 29;
        CLK_CNTR->UART3_CLK = UART_CLK_SOURCE << 28 | 1<<16 | UART_BRG;        
        break;
    }
