...
class uart : public Print
{
  public:
    ...
    uint8_t Transmit(uint8_t *Buff, uint8_t Length);

    // Функции от доброго человека
    int16_t read();                                    //Прочитать симол из буфера приемника. Return: -1 - нет данных для чтения, 0..255 - прочитанный символ
    ...
    bool    isTxFinish(void)               { if((_USART_Port->ISR & USART_ISR_TXE) && (_USART_Port->ISR & USART_ISR_TC)) return true; else return false;}

//    bool isTxFinish(void) { return 0;} // Временная заглушка
