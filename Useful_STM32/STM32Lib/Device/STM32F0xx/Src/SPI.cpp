...
//  _SPI_Port->CR1  |= (0<<SPI_CR1_CPOL_Pos | 0<<SPI_CR1_CPHA_Pos);                         // Режим работы SPI: 0
  SetMode(_Mode);                                                                         // Режим работы по умолчанию 0

  _SPI_Port->CR2  |= (0b1111 << SPI_CR2_DS_Pos);                                          // Так как сброс регистра в "0" не возможен, загоняем его в "1"
  _SPI_Port->CR2  &= ~((0b1111 ^ _DataSize) << SPI_CR2_DS_Pos);                           // Устанавливаем количество передаваемых бит. По умолчанию 8
...
size_t SPI::Transmit(uint16_t Data)
{
  ...
  while(_SPI_Port->SR & SPI_SR_FTLVL){};                                                 // Ждём, пока не освободится FIFO передатчика
  return 0;
}
...
size_t SPI::TransmitReceive(uint16_t *TxBuff, uint16_t *RxBuff, uint8_t Length)
{
  return 0;
}
...
{
  while((_SPI_Port->SR & SPI_SR_FTLVL) && (_SPI_Port->SR & SPI_SR_FRLVL)){};              // Ждём, пока не освободится FIFO передатчика и приёмника
  if(Selection)
