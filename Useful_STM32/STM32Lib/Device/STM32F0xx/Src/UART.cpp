...
void uart::init(uint32_t BaudRate)
{
  ...
  if(_USART_Port == USART1)
  {
    ...
    _SetPull(Pull_Up);
    _SetPinAlternate(GPIOA, GPIO_PIN_9 | GPIO_PIN_10, GPIO_AF1_USART1);
  }
  else if(_USART_Port == USART2)
  {
    ...
    _SetSpeed(Speed_High);
    _SetPull(Pull_Up);
    _SetPinAlternate(GPIOA, GPIO_PIN_2 | GPIO_PIN_3, GPIO_AF1_USART2);
  }

  // Рассчитываем делитель для получения скорости передачи
  _BaudRate = BaudRate;
  Prescaler = APBPrescTable[(RCC->CFGR & RCC_CFGR_PPRE) >> RCC_CFGR_PPRE_Pos];
...
int16_t uart::read()            {/*return ring_get(&rx_fifo)*/return 0;}
...
extern "C"  void USART2_IRQHandler(void)
{
  ...
  if((USART2->ISR & USART_ISR_TXE) && (USART2->CR1 & USART_CR1_TXEIE))              // Если передачтик пуст
  {
    if(RingBuffNumOfItems(&tx_fifo) > 0)                                        // если есть, что передавать, передаём
    {
      tmp = RingBuffGet(&tx_fifo);
      USART2->TDR = (uint8_t)(tmp & 0xFF);
    }
    else                                                                        // если передавать нечего
    {
