...
void _SetPin(GPIO_TypeDef *GPIOx_Set, uint16_t GPIO_Pin_Set, uint8_t IO_Set, uint8_t PULL_Set)
{
  ...
  if(_GPIOx_Set == GPIOA) RCC->AHB1ENR |= RCC_AHB1ENR_GPIOAEN;
  else if(_GPIOx_Set == GPIOB) RCC->AHB1ENR |= RCC_AHB1ENR_GPIOBEN;
  else if(_GPIOx_Set == GPIOC) RCC->AHB1ENR |= RCC_AHB1ENR_GPIOCEN;
  else if(_GPIOx_Set == GPIOD) RCC->AHB1ENR |= RCC_AHB1ENR_GPIODEN;
  else if(_GPIOx_Set == GPIOE) RCC->AHB1ENR |= RCC_AHB1ENR_GPIOEEN;

#if defined(STM32F407xx)
  else if(_GPIOx_Set == GPIOF) RCC->AHB1ENR |= RCC_AHB1ENR_GPIOFEN;
  else if(_GPIOx_Set == GPIOG) RCC->AHB1ENR |= RCC_AHB1ENR_GPIOGEN;
#endif

  else if(_GPIOx_Set == GPIOH) RCC->AHB1ENR |= RCC_AHB1ENR_GPIOHEN;
...
bool _DigitalReadBit(GPIO_TypeDef *GPIOx_Set, uint16_t GPIO_Pin_Set)
{
  if(GPIOx_Set->IDR & GPIO_Pin_Set)
    return 1;                                                                   // Если установлен, возвращаем 1 иначе 0
  else
    return 0;
}
