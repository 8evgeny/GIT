...
FSMC16_TFT::FSMC16_TFT():GFX_TFT(240, 320)
{
...
void FSMC16_TFT::FSMC16_GPIO_Speed(uint8_t Speed)
{
  _FSMC_GPIO_Speed = Speed;
}

void FSMC16_TFT::FSMC16_Speed(uint8_t AddressSetupTime, uint8_t AddressHoldTime, uint8_t DataSetupTime, uint8_t BusTurnAroundDuration,
...
void FSMC16_TFT::FSMC16_Init(uint8_t Ax_Pin, uint8_t NEx_Pin)
{
  _FSMC_Ax  = Ax_Pin;
  _FSMC_NEx = NEx_Pin;
  ...
  Init();
  _SetSpeed(Speed_Low);                                               // Устанавливаем обратно скорость портов на Low. Только высокоскоростные устанавливают
                                                                      // что им нравится.
}

void FSMC16_TFT::FSMC16_GPIO_Init()
{
  uint16_t Temp = 0;
...
//      _TFT_Base = 0x6C000000;                                       // Скорее всего биты 26, 27 влияют на то какой NE выбран
      _TFT_Ax   = 0x000007FE;
      break;
    case FSMC_A11:
...
void FSMC16_TFT::_setBaseAddress(uint32_t Base, uint32_t Data)
{
  LCD = ((LCD_TypeDef*) ((uint32_t) (Base | Data)));
}

void FSMC16_TFT::Init()
{
  if(Flag_Reset)
  {
...
void FSMC16_TFT::setWindow(uint16_t sx, uint16_t sy, uint16_t width, uint16_t height)
{
  WR_REG(CMD_SET_X);
  WR_DATA(sx>>8);
  WR_DATA(sx&0XFF);
...
void FSMC16_TFT::fillScreen(uint16_t color)
{
  uint32_t index=0;
...
void FSMC16_TFT::setRotation(uint8_t x)
{
  WR_REG(MADCTL);
  rotation = x % 8;
  switch (rotation)
...
void FSMC16_TFT::CursorXY(uint16_t x, uint16_t y)
{
  WR_REG(CMD_SET_X);
  WR_DATA(x>>8);
  WR_DATA(x&0XFF);
...
void FSMC16_TFT::drawPixel(uint16_t x, uint16_t y, uint16_t color)
{
 ...
 */
void FSMC16_TFT::drawRGBBitmap(uint16_t x, uint16_t y, const uint16_t *bitmap, uint16_t w, uint16_t h)
{
  uint32_t index = 0;
  uint32_t totalpoint;
...
void FSMC16_TFT::drawFillFastRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color)
{
  uint32_t index = 0;
